...
const opcionesDataTable = {
    ...
    dom: 'Bfrltip', //Para que se vean los botones
    buttons: [
        {
            extend: 'excelHtml5',
...
const tablaCargada = async()=>{
    ...
    dataTable = $("#tablaClientes").DataTable(opcionesDataTable);
...
const mostrarDatos = async =>{
    try{        
        ...
        clientes.forEach((clientes, index) => {
            ...
            contensBody+= `
            <tr id="fila_` + id + `">
                <td class="centered border bg-primary-200 text-white" id="codigoCliente_` + id + `">${clientes.codigoCliente}</td>
                <td class="centered border bg-primary-200 text-white" id="nombreCliente_` + id + `">${clientes.nombreCliente}</td>
                <td class="centered border bg-primary-200 text-white" id="apellidoCliente_` + id + `">${clientes.apellidoCliente}</td>
                <td class="centered border bg-primary-200 text-white" id="ciCliente_` + id + `">${clientes.ciCliente}</td>
                <td class="centered border bg-primary-200 text-white" id="sexoCliente_` + id + `">${clientes.sexoCliente}</td>
                <td class="centered border bg-primary-200 text-white" id="fechaNCliente_` + id + `">${clientes.fechaNCliente}</td>
                <td class="centered border bg-primary-200 text-white" id="direccionCliente_` + id + `">${clientes.direccionCliente}</td>
                <td class="centered border bg-primary-200 text-white" id="rucCliente_` + id + `">${clientes.rucCliente}</td>
                <td class="centered border bg-primary-200 text-white">
                    <button onclick="editarFilaCliente(${id});" data-bs-toggle="modal" data-bs-target="#modalClientes" class="btn btn-success bi bi-pencil-square"></button>
                    <button onclick="eliminarFilaCliente(${id});" class="btn btn-danger bi bi-trash3"></button>
                </td>
            </tr>`;
            tablaClientesBody.innerHTML = contensBody; 
        });
    }catch(ex){
        alert(ex);
...
function editarFilaCliente(idFila){
    var codCliente = document.getElementById('codigoCliente_' + idFila);
    var pos = buscarPosCliente(codCliente.textContent);
    localStorage.setItem('posCliente', pos);
    
    document.getElementById('codCliente').value = clientes[pos].codigoCliente;
    document.getElementById('ciCliente').value = clientes[pos].ciCliente;
    document.getElementById('nomCliente').value = clientes[pos].nombreCliente;
    document.getElementById('apeCliente').value = clientes[pos].apellidoCliente;
    document.getElementById('fechaNCliente').value = clientes[pos].fechaNCliente.replace(/\//g, '-');
    document.getElementById('dirCliente').value = clientes[pos].direccionCliente;
    document.getElementById('telCliente').value = clientes[pos].telefono;
    document.getElementById('rucCliente').value = clientes[pos].rucCliente;
    document.getElementById('nombreRazon').value = clientes[pos].nombreRazonSocial;
    document.getElementById('fechaCCliente').textContent = formatearFechaDT(clientes[pos].fechaCreacionCliente);
    document.getElementById('fechaMCliente').textContent = formatearFechaDT(clientes[pos].fechaModificacionCliente);
    document.getElementById('usuarioCCliente').textContent = clientes[pos].usuarioCreacion;
    document.getElementById('usuarioMCliente').textContent = clientes[pos].usuarioModificacion;

    var selectSexo = document.getElementById('sexoCliente');
    for (var i = 0; i < selectSexo.options.length; i++) {
        if (selectSexo.options[i].value === clientes[pos].sexoCliente) {
            selectSexo.options[i].selected = true;
            break;
        }
    }
}
...
async function eliminarFilaCliente(idFila){
    // Obtener la fila actual
    var fila = document.getElementById('fila_' + idFila);
    var codCliente = document.getElementById('codigoCliente_' + idFila).textContent;
    var posEliminado = buscarPosCliente(codCliente);
    if(!verificarUsoCliente(codCliente)){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                clientes.splice(posEliminado, 1);
                localStorage.setItem('lSClientes', JSON.stringify(clientes));
                // Eliminar la fila
                fila.remove();
                tablaCargada();
                Swal.fire({
...
function codClienteCarga(){
    document.getElementById('codCliente').value = codigoMayor() + 1;
    document.getElementById('labelAgregarModificar').textContent = 'Agregar Cliente';
...
function codigoMayor(){
    ...
    for(let x = 0; x < clientes.length; x++){
        if(x == 0){
            may = clientes[x].codigoCliente;
        }else{
            if(clientes[x].codigoCliente > may){
                may = clientes[x].codigoCliente;
            }
        }
    }
...
function guardarCliente(event){
    ...
    if(document.getElementById('formClientes').checkValidity()){
        let codigoCliente = parseInt(document.getElementById('codCliente').value);
        let modificacion = buscarPosCliente(codigoCliente);
    
        if(modificacion !== -1){
            clientes[modificacion].ciCliente = parseInt(document.getElementById('ciCliente').value);
            clientes[modificacion].nombreCliente = document.getElementById('nomCliente').value.toUpperCase();
            clientes[modificacion].apellidoCliente = document.getElementById('apeCliente').value.toUpperCase();
            clientes[modificacion].fechaNCliente = document.getElementById('fechaNCliente').value;
            clientes[modificacion].sexoCliente = document.getElementById('sexoCliente').value;
            clientes[modificacion].telefono = document.getElementById('telCliente').value;
            clientes[modificacion].rucCliente = document.getElementById('rucCliente').value;
            clientes[modificacion].nombreRazonSocial = document.getElementById('nombreRazon').value.toUpperCase();
            clientes[modificacion].direccionCliente = document.getElementById('dirCliente').value;
            clientes[modificacion].fechaModificacionCliente = fechaHoraActualReturn();
            clientes[modificacion].usuarioModificacion = usuLogeado.usuario;
    
            localStorage.setItem('lSClientes', JSON.stringify(clientes));
            tablaCargada();
            $('#modalClientes').modal('hide'); 
        }else{  // Se guarda como nuevo cliente
            let nuevoCliente = {
                codigoCliente: parseInt(document.getElementById('codCliente').value),
                ciCliente: parseInt(document.getElementById('ciCliente').value),
                nombreCliente: document.getElementById('nomCliente').value.toUpperCase(),
                apellidoCliente: document.getElementById('apeCliente').value.toUpperCase(),
                sexoCliente: document.getElementById('sexoCliente').value,
                fechaNCliente: document.getElementById('fechaNCliente').value,
                direccionCliente: document.getElementById('dirCliente').value,
                telefono: document.getElementById('telCliente').value,
                rucCliente: document.getElementById('rucCliente').value,
                nombreRazonSocial: document.getElementById('nombreRazon').value.toUpperCase(),
                fechaCreacionCliente: fechaHoraActualReturn(),
...
function verificarUsoCliente(cod){
    let uso = false;
    ventas.forEach((ventas, index) => {
        if(ventas.codCliente == cod){
            uso = true;
        }
    })
    return uso;
