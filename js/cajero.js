//Ultima modificacion 02/06/2024 23:15

// Seccion de recuperacion de datos de la base de datos 
let clientes = JSON.parse(localStorage.getItem('lSClientes'));
let articulos = JSON.parse(localStorage.getItem('lSArticulos'));
let ventas = JSON.parse(localStorage.getItem('lSVentas'));
let timbrados = JSON.parse(localStorage.getItem('lSTimbrados'));
let planillas = JSON.parse(localStorage.getItem('lSPlanillas'));
let usuLogeado = JSON.parse(sessionStorage.getItem('usuarioLogeado'));
let usuarios = JSON.parse(localStorage.getItem('lSUsuarios'));
let tiposValor = JSON.parse(localStorage.getItem('lSTiposValor'));
let articulosPreVenta = JSON.parse(localStorage.getItem('lSArticulos'));


// Variables bandera y otros
let tablaClienteIniciada = false;
let tablaPlanillaIniciada = false;
let tablaInformeVentasIniciada = false;
let tablaCargaIniciada = false;
let tablaTipoValorIniciada = false;
let contAutorizacion=123;
let tempNumFac, tempNumCaja, tempNumSucursal;
let ventaGuardada;
let idFilaPlanillaHabilitada;
let artCargados = 0;
let nuevaP = false;
let editarFila = false;
let planillaCerrada = false; //para saber si la planilla a editar ya estaba cerrada o no
let artC = [];
let infoVentas = [];
let totalVentaCarga = 0; // Variable vandera donde se guarda el subTotal de los articulos cargados para poder usar en la funcion finalizar Venta

//para calcular el total de la venta
let totalVenta = 0;
let total10 = 0;
let total5 = 0;
let totalExenta = 0;
let ivaTotal5 = 0;
let ivaTotal10 = 0;
let totalIva = 0;




window.addEventListener("load", async () => {
    const modalSesionRequerida = new bootstrap.Modal(document.getElementById('modalIniciarSesion'));
    modalSesionRequerida.show();
    if (!sessionStorage.getItem('usuarioLogeado')) {
        Swal.fire({
            position: "center",
            icon: "info",
            title: "Inicio de sesión requerido",
            showConfirmButton: false,
            timer: 1500
        }).then(() => {
            window.location.href = 'index.html';
        });

    } else {
        if(usuLogeado.rol == 'Admin'){
            document.getElementById('btnTipoValor').hidden = false;
        }
        await tablaCargada();
        await tablaArtCargada();
        await tablaTipoValorCargada();
        cargarDatosUsuario();
        nuevo();
        modalSesionRequerida.hide();
    }
});

document.addEventListener('keydown', function(event) {
    if (event.key === 'F1') {
        event.preventDefault();
        if (document.activeElement === document.getElementById('codCliente')) {
            tablaBuscarClienteCargada();
            $('#modalBuscarCliente').modal('show');
        }
    }
});


function editarUsuario(){
    document.getElementById('nombreEditUsuario').value = usuLogeado.nombre;
    document.getElementById('apellidoEditUsuario').value = usuLogeado.apellido;
    document.getElementById('correoEditUsuario').value = usuLogeado.correo;
    document.getElementById('telefonoEditUsuario').value = usuLogeado.telefono;
    document.getElementById('usuarioEditUsuario').value = usuLogeado.usuario;
    $('#modalEditarUsuario').modal('show');
}
const guardarEditUsuario = async(event) => {
    const forms = document.querySelectorAll('.needs-validation')
    Array.from(forms).forEach(form => {
        form.addEventListener('submit', event => {
        if (!form.checkValidity()) {
            event.preventDefault()
            event.stopPropagation()
        }

        form.classList.add('was-validated')
        }, false)
    })
    event.preventDefault();
    if(document.getElementById('formEditarUsuario').checkValidity()){
        let pos = buscarPosUsuario(usuLogeado.codigo);
        let contraseñaAnterior = document.getElementById('contAnterior').value;
        let nuevaC = document.getElementById('nuevaCont').value;
        let repetNuevaC = document.getElementById('repetirNuevaCont').value;
        if(contraseñaAnterior !== ''){
            if(contraseñaAnterior === usuLogeado.contraseña){
                if(nuevaC !== '' && nuevaC === repetNuevaC){
                    usuarios[pos].nombre = document.getElementById('nombreEditUsuario').value;
                    usuarios[pos].apellido = document.getElementById('apellidoEditUsuario').value;
                    usuarios[pos].correo = document.getElementById('correoEditUsuario').value;
                    usuarios[pos].usuario = document.getElementById('usuarioEditUsuario').value;
                    usuarios[pos].contraseña = nuevaC;
                    usuarios[pos].telefono = document.getElementById('telefonoEditUsuario').value;
                    Swal.fire({
                        icon: 'success',
                        title: 'Exito',
                        text: 'Usuario Guardado',
                        timer: 1500
                    })
    
                    usuLogeado = usuarios[pos];
                    sessionStorage.setItem('usuarioLogeado', JSON.stringify(usuLogeado));
                    localStorage.setItem('lSUsuarios', JSON.stringify(usuarios));
                    $('#modalEditarUsuario').modal('hide');
                }else{
                    Swal.fire({
                        title: 'Las contraseñas no coinciden',
                        icon: 'warning',
                        timer: 1500
                    })
                }
            }else{
                Swal.fire({
                    title: 'Contraseña Incorrecta',
                    text: 'Ingrese su contraseña anterior',
                    icon: 'error',
                    showConfirmButton: true,
                    timer: 2000
                })
            }
        }else{        
            usuarios[pos].nombre = document.getElementById('nombreEditUsuario').value;
            usuarios[pos].apellido = document.getElementById('apellidoEditUsuario').value;
            usuarios[pos].correo = document.getElementById('correoEditUsuario').value;
            usuarios[pos].usuario = document.getElementById('usuarioEditUsuario').value;
            usuarios[pos].telefono = document.getElementById('telefonoEditUsuario').value;
    
            usuLogeado = usuarios[pos];
            sessionStorage.setItem('usuarioLogeado', JSON.stringify(usuLogeado));
    
            localStorage.setItem('lSUsuarios', JSON.stringify(usuarios));
            await Swal.fire({
                title: 'Usuario Guardado',
                icon: 'success',
                timer: 1500
            })
            $('#modalEditarUsuario').modal('hide');
        }
        cargarDatosUsuario();
    }

}
function buscarPosUsuario(cod){
    pos = -1;
    usuarios.forEach((usuarios, index) => {
        if(usuarios.codigo == cod){
            pos = index;
        }
    })
    return pos;
}


// Seccion de Datatables -------------------------------------------------------------------------------------------------------
const tablaArtCargada = async()=>{
    // Datatable Articulos Cargados
    if(tablaCargaIniciada){
        dataTableCarga.destroy();
    }
    await mostrarDatosCarga();
    dataTableCarga = $("#tablaCargaArticulos").DataTable(opcionesDataTableCarga);
    tablaCargaIniciada = true;
};

const tablaInfoVentasCargada = async() =>{
    // Datatable Informe Ventas
    if(tablaInformeVentasIniciada){
        dataTableInfoVentas.destroy();
    }
    await mostrarDatosInfoVentas();
    dataTableInfoVentas = $("#tablaInformeVentas").DataTable(opcionesDataTableInfoVentas);
     // Detectar cuando se hace clic en el botón de imprimir
    dataTableInfoVentas.buttons().container().on('click', '.buttons-print', function() {
        funcionesPrint();
    });
    tablaInformeVentasIniciada = true;
};
const tablaBuscarClienteCargada = async() =>{
    // Datatable Buscar Cliente
    if(tablaClienteIniciada){
        dataTableCliente.destroy();
    }
    await mostrarDatosCliente();
    dataTableCliente = $("#tablabuscarClientes").DataTable(opcionesDataTableCliente);
    //vincula el input buscar cliente a la funcion de busqueda de datatables
    $('#buscarCliente').on('keyup', function() {
        dataTableCliente.search(this.value).draw();
    });
    tablaIniciadaCliente = true;
}

const tablaCargada = async()=>{
    // Datatable Articulos Cargados
    if(tablaCargaIniciada){
        dataTableCarga.destroy();
    }
    await mostrarDatosCarga();
    dataTableCarga = $("#tablaCargaArticulos").DataTable(opcionesDataTableCarga);
    tablaCargaIniciada = true;
};
const tablaPlanillasCargada = async () =>{
    // Datatable Planillas
    if(tablaPlanillaIniciada){
        dataTablePlanilla.destroy();
    }
    await mostrarDatosPlanilla();
    dataTablePlanilla = $("#tablaPlanillas").DataTable(opcionesDataTablePlanilla);
    tablaPlanillaIniciada = true;
}
const tablaPlanillasFiltradaCargada = async () =>{
    // Datatable Planillas
    if(tablaPlanillaIniciada){
        dataTablePlanilla.destroy();
    }
    await mostrarDatosPlanillaFiltrada();
    dataTablePlanilla = $("#tablaPlanillas").DataTable(opcionesDataTablePlanilla);
    tablaPlanillaIniciada = true;
}



const funcionesPrint = async () => {
    let posPlanilla = buscarPosNumPlanilla(document.getElementById('numPlanilla').value);
    if(posPlanilla !== -1){
        let posUsuario = buscarPosNomUsuario(planillas[posPlanilla].cajero);
        if(posUsuario !== -1){
            document.getElementById('tituloPagina').textContent = 'Reporte de Ventas - ' + usuarios[posUsuario].nombre + ' ' + usuarios[posUsuario].apellido;
        }
    }
}
async function cerrarSesion(){
    Swal.fire({
        title: "Cerrar Sesion?",
        text: "",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#d33",
        cancelButtonColor: "#3085d6",
        confirmButtonText: "Si",
        cancelButtonText: "No"
    }).then((result) => {
        if (result.isConfirmed) {
            window.location.href = 'index.html';
            sessionStorage.clear();
        }
    });
}
function volver(){  
    let user = JSON.parse(sessionStorage.getItem('usuarioLogeado'));
    switch (user.rol) {
        case 'Admin':
            window.location.href = 'menuAdmin.html';
            break;
        case 'Cajero':
                window.location.href = 'menuCajero.html';
            break;
        case 'Stock':
                window.location.href = 'menuStock.html';
            break;
        default:
            // Código para el caso por defecto
    }
}




// Seccion de PLANILLAS -----------------------------------------------------------------------------------------------------------
const opcionesDataTablePlanilla = {
    columnDefs:[{className:"centered border", targets:[0,1,2,3,4,5,6,7]}],
    pageLength: 5, // Cantidad de registros que se va a ver
    destroy: true,  // Para que la tabla sea destruible
    dom: '', //Para que 
    language:{
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};
const mostrarDatosPlanillaFiltrada = async () => {
    try {     
        let cant = [];   
        let inicio = document.getElementById('fechaInicio').value;
        let fin = document.getElementById('fechaFin').value; 
        let contensBody = ``;
        planillas.forEach((planilla, index) => {
            let id = index + 1;
            if(planilla.fechaHoraApertura >= inicio && planilla.fechaHoraCierre <= fin){
                cant.push(planilla);
                contensBody += `
                <tr id="filaPlanilla_${id}">
                    <td hidden>${id}</td>
                    <td id="codigoPlanilla_${id}">${planilla.codPlanilla}</td>
                    <td><input type="text" class="form-control text-center" id="numPlanilla_${id}" value="${planilla.numPlanilla}" disabled></td>
                    <td><input type="datetime-local" class="form-control text-center" id="fechaApertura_${id}" value="${planilla.fechaHoraApertura.replace(/\//g, '-').slice(0, 16)}" disabled></td>
                    <td><input type="datetime-local" class="form-control text-center" id="fechaCierre_${id}" value="${planilla.fechaHoraCierre.replace(/\//g, '-').slice(0, 16)}" disabled></td>
                    <td>
                        <div class="form-floating">
                            <select class="form-control text-dark" id="selecCajeros_${id}" disabled>
                                <option value="Indefinido">Indefinido</option>
                            </select>
                            <label class="mx-2 text-dark" for="selecCajeros_${id}">Cajero</label>
                        </div>
                    </td>
                    <td><input type="text" class="form-control text-center" id="usuarioReapertura_${id}" value="${planilla.usuarioReapertura}" disabled></td>                
                    <td>
                        <button onclick="modificarFilaPlanilla(${id})" class="btn btn-success bi bi-pencil-square"></button>
                        <button onclick="eliminarFilaPlanilla(${id});" class="btn btn-danger bi bi-trash3"></button>
                    </td>
                </tr>`;
            }
        });

        tablaPlanillaBody.innerHTML = contensBody;

        cant.forEach((cant, index) => {
            let id = index + 1;
            llenarSelectCajeros(id);

            var cajero = document.getElementById('selecCajeros_' + id);
            for (var i = 0; i < cajero.options.length; i++) {
                if (cajero.options[i].value === cant.cajero) {
                    cajero.options[i].selected = true;
                    break;
                }
            }
        });
    } catch (ex) {
        alert(ex);
        console.log('No se pudo cargar la planilla');
    }
};
const mostrarDatosPlanilla = async () => {
    try {        
        let contensBody = ``;
        planillas.forEach((planilla, index) => {
            let id = index + 1;

            contensBody += `
            <tr id="filaPlanilla_${id}">
                <td hidden>${id}</td>
                <td id="codigoPlanilla_${id}">${planilla.codPlanilla}</td>
                <td><input type="text" class="form-control text-center" id="numPlanilla_${id}" value="${planilla.numPlanilla}" disabled></td>
                <td><input type="datetime-local" class="form-control text-center" id="fechaApertura_${id}" value="${planilla.fechaHoraApertura.replace(/\//g, '-').slice(0, 16)}" disabled></td>
                <td><input type="datetime-local" class="form-control text-center" id="fechaCierre_${id}" value="${planilla.fechaHoraCierre.replace(/\//g, '-').slice(0, 16)}" disabled></td>
                <td>
                    <div class="form-floating">
                        <select class="form-control text-dark" id="selecCajeros_${id}" disabled>
                            <option value="Indefinido">Indefinido</option>
                        </select>
                        <label class="mx-2 text-dark" for="selecCajeros_${id}">Cajero</label>
                    </div>
                </td>
                <td><input type="text" class="form-control text-center" id="usuarioReapertura_${id}" value="${planilla.usuarioReapertura}" disabled></td>                
                <td>
                    <button onclick="modificarFilaPlanilla(${id})" class="btn btn-success bi bi-pencil-square"></button>
                    <button onclick="eliminarFilaPlanilla(${id});" class="btn btn-danger bi bi-trash3"></button>
                </td>
            </tr>`;
        });

        tablaPlanillaBody.innerHTML = contensBody;

        planillas.forEach((planillas, index) => {
            let id = index + 1;
            llenarSelectCajeros(id);

            var cajero = document.getElementById('selecCajeros_' + id);
            for (var i = 0; i < cajero.options.length; i++) {
                if (cajero.options[i].value === planillas.cajero) {
                    cajero.options[i].selected = true;
                    break;
                }
            }
        });
    } catch (ex) {
        alert(ex);
        console.log('No se pudo cargar la planilla');
    }
};
function modificarFilaPlanilla(id){
    if(nuevaP == false){
        if(document.getElementById('fechaCierre_' + id).value !== ''){
            planillaCerrada = true;
        }else{
            planillaCerrada = false;
        }
        habilitarFila(id);
    }
};
function abrirPlanilla(){
    document.getElementById('fechaInicio').value = fechaActual_5dias();
    document.getElementById('fechaFin').value = fechaActual();
    tablaPlanillasCargada();
    $('#modalPlanillas').modal('show');
}
function habilitarFila(id) {
    idFilaPlanillaHabilitada = id;
    let cantidadFila = document.getElementById('tablaPlanillas').rows.length;
    var modal = document.getElementById('modalPlanillas');
    var modalBS = bootstrap.Modal.getInstance(modal); // Obtener la instancia del modal de Bootstrap

        if(usuLogeado.rol == 'Admin'){
            
            document.getElementById('fechaApertura_' + id).disabled = false;
            document.getElementById('fechaCierre_' + id).disabled = false;
            document.getElementById('selecCajeros_' + id).disabled = false;
            editarFila = true;
    
            for(let i = 1; i < cantidadFila; i++){
                if(i !== id){
                    document.getElementById('fechaApertura_' + i).disabled = true;
                    document.getElementById('fechaCierre_' + i).disabled = true;
                    document.getElementById('selecCajeros_' + i).disabled = true;
                }
            }
            // Se cierra el modal Planillas
            document.getElementById('modalPlanillas').addEventListener('hidden.bs.modal', () => {
                editarFila = false;
                desabilitarFila(id);
            })
        }else{
            if(usuLogeado.usuario == document.getElementById('selecCajeros_' + id).value && planillaCerrada == false){
                document.getElementById('fechaCierre_' + id).disabled = false;
                editarFila = true;
        
                for(let i = 1; i < cantidadFila; i++){
                    if(i !== id){
                        document.getElementById('fechaCierre_' + i).disabled = true;
                    }
                }
                // Se cierra el modal Planillas
                document.getElementById('modalPlanillas').addEventListener('hidden.bs.modal', () => {
                    editarFila = false;
                    desabilitarFila(id);
                })
            }else{
                Swal.fire({
                    position: "center",
                    icon: "info",
                    title: "No estas autorizado para Modificar planillas",
                    showConfirmButton: true,
                    timer: 1500
                })
            }
        }
};
function desabilitarFila(id) {
    if(id){
        document.getElementById('fechaApertura_' + id).disabled = true;
        document.getElementById('fechaCierre_' + id).disabled = true;
        document.getElementById('selecCajeros_' + id).disabled = true;
    }
};
function llenarSelectCajeros(id) {
    var select = document.getElementById('selecCajeros_' + id);
    let cont = 0;
    for(let x = 0; x < select.options.length; x++){
        cont++;
    }

    if(cont <= 1){
        usuarios.forEach(function(usuarios){
            if(usuarios.rol == 'Cajero' || usuarios.rol == 'Admin'){
                var option = document.createElement("option");
                option.text = usuarios.usuario;
                option.value = usuarios.usuario;
                select.appendChild(option);
            }
        })

    }
};
function nuevaPlanilla() {
    if(nuevaP == false && editarFila == false){
        nuevaP = true;
        try {   
            let tabla = document.getElementById('tablaPlanillas');
            let id = tabla.getElementsByTagName('tbody')[0].rows.length + 1;
            let codPlanilla = codSigPlanilla();
            let numPlanilla = codSigNumPlanilla();
            let nuevaFila = `
            <tr id="filaPlanilla_${id}" class="centered border">
                <td hidden>${id}</td>
                <td id="codigoPlanilla_${id}">${codPlanilla}</td>
                <td><input type="text" class="form-control text-center" id="numPlanilla_${id}" value="${numPlanilla}" disabled></td>
                <td><input type="datetime-local" class="form-control text-center" id="fechaApertura_${id}" value="${fechaHoraActualReturn()}"></td>
                <td><input type="datetime-local" class="form-control text-center" id="fechaCierre_${id}"></td>
                <td>
                    <div class="form-floating">
                        <select class="form-control text-dark" id="selecCajeros_${id}">
                            <option value="Indefinido">Indefinido</option>
                        </select>
                        <label class="mx-2 text-dark" for="selecCajeros_${id}">Cajero</label>
                    </div>
                </td>
                <td><input type="text" class="form-control text-center" id="usuarioReapertura_${id}" disabled></td>                
                <td>
                    <button onclick="habilitarFila(${id})" class="btn btn-success bi bi-pencil-square"></button>
                    <button onclick="eliminarFilaPlanilla(${id});" class="btn btn-danger bi bi-trash3"></button>
                </td>
            </tr>`;

            // Agregar la nueva fila al cuerpo de la tabla sin reemplazar todo el contenido
            let cuerpoTabla = tabla.getElementsByTagName('tbody')[0];
            cuerpoTabla.insertAdjacentHTML('beforeend', nuevaFila);

            // Llenar el select de cajeros para la nueva fila
            llenarSelectCajeros(id);

            // Selecciona el cajero segun el usuario logeado
            var cajero = document.getElementById('selecCajeros_' + id);
            for (var i = 0; i < cajero.options.length; i++) {
                if (cajero.options[i].value === usuLogeado.usuario) {
                    cajero.options[i].selected = true;
                    break;
                }
            }

            // Se cierra el modal Planillas
            document.getElementById('modalPlanillas').addEventListener('hidden.bs.modal', () => {
                verifivarPlanilla();
                if(nuevaP == true){
                    let tabla = document.getElementById('tablaPlanillas');
                    if (tabla.rows.length > 1) tabla.deleteRow(-1);
                }
                nuevaP = false;
            });
        } catch (ex) {
            alert(ex);        
        }
    }
};
function guardarPlanilla(){ 
    try{
        var modalBS = bootstrap.Modal.getInstance(document.getElementById('modalPlanillas')); // Obtener la instancia del modal de Bootstrap
        if(nuevaP == true){
            // Se guarda como nueva planilla
            let id = document.getElementById('tablaPlanillas').rows.length-1;
            let fechaA = document.getElementById('fechaApertura_' + id).value;
            let fechaC = document.getElementById('fechaCierre_' + id).value;
            if(fechaA !== ''){
                if(verificarFechaCierrePlanilla(fechaA, fechaC) == true || fechaC == ''){
                    if(document.getElementById('selecCajeros_' + id).value !== 'Indefinido'){
                        let nuevaPlanilla ={
                            codPlanilla: parseInt(document.getElementById('codigoPlanilla_' + id).textContent),
                            numPlanilla: parseInt(document.getElementById('numPlanilla_' + id).value),
                            fechaHoraApertura: document.getElementById('fechaApertura_' + id).value,
                            fechaHoraCierre: document.getElementById('fechaCierre_' + id).value,
                            fechaHoraCreacion: fechaHoraActualReturn(),
                            cajero: document.getElementById('selecCajeros_' + id).value,
                            usuarioCreacion: usuLogeado.usuario,
                            usuarioReapertura: '',
                        };
            
                        planillas.push(nuevaPlanilla);
                        localStorage.setItem('lSPlanillas', JSON.stringify(planillas));
                        nuevaP = false;
                        $('#modalPlanillas').modal('hide');
            
                        // Se cierra el modal Planillas
                        document.getElementById('modalPlanillas').addEventListener('hidden.bs.modal', () => {
                            desabilitarFila(id);
                            verifivarPlanilla();
                        });
                    }else{
                        Swal.fire({
                            position: "center",
                            icon: "info",
                            title: "Debes seleccionar el cajero",
                            showConfirmButton: true,
                            timer: 1500
                        })
                    } 
                }else{
                    Swal.fire({
                        position: "center",
                        icon: "error",
                        title: "Fecha de cierre invalida",
                        text: "La fecha de cierre no puede ser menor a la fecha de apertura",
                        showConfirmButton: true,
                        timer: 2500
                    }) 
                }
            }else{
                Swal.fire({
                    position: "center",
                    icon: "info",
                    title: "La fecha de apertura no esta definida",
                    showConfirmButton: true,
                    timer: 1500
                })
            }
        }else{
            // se guarda como planilla modificada
            if(editarFila == true){
                let idFila = idFilaPlanillaHabilitada;
                let fechaA = document.getElementById('fechaApertura_' + idFila).value;
                let fechaC = document.getElementById('fechaCierre_' + idFila).value;
                if(fechaA !== ''){
                    if(verificarFechaCierrePlanilla(fechaA, fechaC) == true || fechaC == ''){
                        if(document.getElementById('selecCajeros_' + idFila).value !== 'Indefinido'){
                            let codPlanilla = buscarPosPlanilla(document.getElementById('codigoPlanilla_' + idFila).textContent);
                            let posPlanilla = buscarPosPlanilla(codPlanilla);

                            planillas[posPlanilla].fechaHoraCierre = document.getElementById('fechaCierre_' + idFila).value;
                            planillas[posPlanilla].cajero = document.getElementById('selecCajeros_' + idFila).value;

                            //si la planilla ya estaba cerrada y se abre de vuelta se guarda el usario de reapertura
                            if(planillaCerrada == true && document.getElementById('fechaCierre_' + idFila).value == ''){
                                planillas[posPlanilla].usuarioReapertura = usuLogeado.usuario;
                                document.getElementById('usuarioReapertura_' + idFila).value = usuLogeado.usuario;
                            }
                            
                            localStorage.setItem('lSPlanillas', JSON.stringify(planillas));  //se guarda la planilla modificada
                            modalBS.hide(); // Ocultar el modal
                        
                            // Se cierra el modal Planillas
                            document.getElementById('modalPlanillas').addEventListener('hidden.bs.modal', () => {
                                verifivarPlanilla();
                                desabilitarFila(idFila);
                            })
                        }else{
                            Swal.fire({
                                position: "center",
                                icon: "info",
                                title: "Debes seleccionar el cajero",
                                showConfirmButton: true,
                                timer: 1500
                            })
                        }
                    }else{
                        Swal.fire({
                            position: "center",
                            icon: "error",
                            title: "Fecha de cierre invalida",
                            text: "La fecha de cierre no puede ser menor a la fecha de apertura",
                            showConfirmButton: true,
                            timer: 2500
                        }) 
                    }
                }else{
                    Swal.fire({
                        position: "center",
                        icon: "info",
                        title: "La fecha de apertura no esta definida",
                        showConfirmButton: true,
                        timer: 1500
                    })
                }  
            }else{
                modalBS.hide(); // Ocultar el modal
            }

                      
        }
    }catch(ex){
        alert(ex);
        console.log('No se pudo guardar');
    }
};
function eliminarFilaPlanilla(idFila){
    if(!verificarPlanillasVentas('numPlanilla_' + idFila)){
        if(usuLogeado.rol == 'Admin')  {    
            if(nuevaP == false && editarFila == false){
                var modal = document.getElementById('modalPlanillas');
                var modalBS = bootstrap.Modal.getInstance(modal); // Obtener la instancia del modal de Bootstrap    
                Swal.fire({
                    title: "Eliminar Planilla?",
                    text: "Una vez eliminado no se puede recuperar",
                    icon: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#d33",
                    cancelButtonColor: "#3085d6",
                    confirmButtonText: "Si, Eliminar"
                }).then((result) => {
                    if (result.isConfirmed) {
                        // Obtener la fila actual
                        var fila = document.getElementById('filaPlanilla_' + idFila);
                        let codPlanilla = document.getElementById('codigoPlanilla_' + idFila).textContent;
                        let posPlanilla = buscarPosPlanilla(codPlanilla);
                        
                        planillas.splice(posPlanilla, 1);  //elimina la planilla en esa posicion
                        localStorage.setItem('lSPlanillas', JSON.stringify(planillas));
    
                        // Se cierra el modal Planillas
                        document.getElementById('modalPlanillas').addEventListener('hidden.bs.modal', () => {
                            verifivarPlanilla();
                        })
    
                        // Eliminar la fila
                        fila.remove();
                        Swal.fire({
                            title: "Planilla Eliminada",
                            text: "",
                            icon: "success"
                        });
                    }
                    tablaCargada();
                });
            }
        }else{
            Swal.fire({
                position: "center",
                icon: "info",
                title: "No estas autorizado para Eliminar planillas",
                showConfirmButton: true,
                timer: 1500
            })
        }
    }else{
        Swal.fire({
            position: "top",
            icon: "info",
            title: "Planilla en uso, no se puede eliminar",
            showConfirmButton: true,
            timer: 3000
        })
    }    
};
function codSigNumPlanilla(){
    let cod = 0;
    for(let i = 0; i < planillas.length; i++){
        if(i == 0){
            cod = planillas[i].numPlanilla;
        }else{
            if(planillas[i].numPlanilla > cod){
                cod = planillas[i].numPlanilla;
            }
        }
    }
    return parseInt(cod) + 1;
};
function codSigPlanilla(){
    let cod = 0;
    for(let i = 0; i < planillas.length; i++){
        if(i == 0){
            cod = planillas[i].codPlanilla;
        }else{
            if(planillas[i].codPlanilla > cod){
                cod = planillas[i].codPlanilla;
            }
        }
    }
    return parseInt(cod) + 1;
};
function buscarPosPlanilla(cod){
    let pos = -1;
    for(let x = 0; x < planillas.length; x++){
        if(cod == planillas[x].codPlanilla){
            pos = x;
            break;
        }
    }
    return pos;
};
function buscarPosNumPlanilla(cod){
    let pos = -1;
    for(let x = 0; x < planillas.length; x++){
        if(cod == planillas[x].numPlanilla){
            pos = x;
            break;
        }
    }
    return pos;
};
function verifivarPlanilla(){
    let planillaHabilitada = false;
    for (let i = planillas.length - 1; i >= 0; i--) {
        if(planillas[i].fechaHoraCierre == ''){
            if(planillas[i].cajero == usuLogeado.usuario){
                document.getElementById('numPlanilla').value = planillas[i].numPlanilla;
                planillaHabilitada = true;
                break;
            }
        }
    }
    if(!planillaHabilitada){
        document.getElementById('numPlanilla').value = '';
    }
};
function verificarFechaCierrePlanilla(fechaAperrtura, fechaCierre){
    let correcto = false;
    if(new Date(fechaCierre) >= new Date(fechaAperrtura)){
        correcto = true;
    }
    return correcto;
}




// Seccion de CLIENTES --------------------------------------------------------------------------------------------------
const opcionesDataTableCliente = {
    columnDefs:[{className:"centered border", targets:[0,1,2,3,4,5,6]}],
    pageLength: 5, // Cantidad de registros que se va a ver
    destroy: true,  // Para que la tabla sea destruible
    dom: '', //Para que 
    language:{
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};
const mostrarDatosCliente = async =>{
    try{        
        let contensBody = ``;
        clientes.forEach((clientes, index) => {
            let id = index + 1;
            contensBody+= `
            <tr data-bs-dismiss="modal" class="seleccionarFilaCliente" onclick="document.getElementById('codCliente').value = ${clientes.codigoCliente}, buscarCliente()" id="fila_` + id + `">
                <td hidden>${index + 1}</td>
                <td id="codigoCliente_` + id + `">${clientes.codigoCliente}</td>
                <td id="nombreCliente_` + id + `">${clientes.nombreCliente}</td>
                <td id="apellidoCliente_` + id + `">${clientes.apellidoCliente}</td>
                <td id="ciCliente_` + id + `">${clientes.ciCliente}</td>
                <td id="rucCliente_` + id + `">${clientes.rucCliente}</td>
                <td id="rucCliente_` + id + `">${clientes.nombreRazonSocial}</td>
            </tr>`;
            tablaBuscarClientesBody.innerHTML = contensBody; 
        });
    }catch(ex){
        alert(ex);
        console.log('No se pudo cargar');
    }
};
function buscarPosCliente(cod){
    let pos = -1;
    for(let x = 0; x < clientes.length; x++){
        if(cod == clientes[x].codigoCliente){
            pos = x;
            break;
        }
    }
    return pos;
};
function buscarCliente(){
    let codCliente = document.getElementById('codCliente').value;
    let posCliente = -1;
    posCliente = buscarPosCliente(codCliente);

    if(posCliente == -1){
        document.getElementById('rucCliente').value = '44444401-7';
        document.getElementById('nomRazonCliete').value = 'Cliente del Salon';
    }else{
        if(clientes[posCliente].rucCliente !== ''){
            document.getElementById('rucCliente').value = clientes[posCliente].rucCliente;
            document.getElementById('nomRazonCliete').value = clientes[posCliente].nombreRazonSocial;
        }else{
            document.getElementById('rucCliente').value = clientes[posCliente].ciCliente;
            document.getElementById('nomRazonCliete').value = clientes[posCliente].apellidoCliente + ', ' + clientes[posCliente].nombreCliente;
        }
    }
};




// Seccion de Articulos --------------------------------------------------------------------------------------------------
function cargarArticulo(event){
    event.preventDefault();
    let codigo = document.getElementById('codigoArticulo').value;
    let nombre = document.getElementById('nombreArticulo').value;
    let cantidad = document.getElementById('cantidadArticulo').value;
    
    
    if(codigo !== '' && parseInt(codigo) !== 0 && cantidad !== '' && parseInt(cantidad) !== 0 && nombre != 'Articulo no encontrado'){
        if(artC.length < 10){
            let posArt = buscarPosArticulo(codigo);
            if(articulos[posArt].estado_art == 'Activo'){
                if(parseInt(cantidad) <= articulosPreVenta[posArt].stock_art){
                    let nuevaCarga = {
                        codArtCarga: parseInt(codigo),
                        nomArtCarga: nombre,
                        cantArtCarga: parseInt(cantidad),
                        precArtCarga: articulos[posArt].precio_art,
                        ivaArtCarga: articulos[posArt].iva_art
                    }
                    articulosPreVenta[posArt].stock_art -= parseInt(cantidad);   
                    artC.push(nuevaCarga);
                    tablaArtCargada();
        
                    // Limpiar el formulario
                    document.getElementById('formCargarArticulo').reset();        
                    document.getElementById('codigoArticulo').focus();
                }else{
                    Swal.fire({
                        position: "center",
                        icon: "info",
                        title: "Stock insuficiente",
                        showConfirmButton: true,
                        timer: 2000
                    })  
                }
            }else{
                Swal.fire({
                    position: "center",
                    icon: "info",
                    title: "Articulo Inactivo",
                    showConfirmButton: true,
                    timer: 2000
                }).then((result) =>{
                    document.getElementById('formCargarArticulo').reset();
                    document.getElementById('codigoArticulo').focus();
                })
            }
        }else{
            Swal.fire({
                position: "center",
                icon: "info",
                title: "Solo se pueden cargar diez articulos por venta",
                showConfirmButton: true,
                timer: 1500
            })
            document.getElementById('formCargarArticulo').reset();
        }
    }
}
const opcionesDataTableCarga = {
    // columnDefs:[{className:"centered border", targets:[0,1,2,3,4,5,6,7]}],
    pageLength: 10, // Cantidad de registros que se va a ver
    destroy: true,  // Para que la tabla sea destruible
    dom: '', //Para que 
    language:{
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};
const mostrarDatosCarga = async () => {
    try {        
        let contensBody = ``;
        totalExenta = 0, total5 = 0, total10 = 0;
        totalVenta = 0;
        
        if(ventaGuardada == true){
            artC.forEach((artC, index) => {
                let id = index + 1;
                let iva10 = '', iva5 = '', exenta = '';
                parseInt(total5, total10, totalExenta);
                if(artC.ivaArtCarga == '10%'){
                    iva10 = artC.precArtCarga * artC.cantArtCarga;
                    total10 = parseInt(total10) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                }else{
                    if(artC.ivaArtCarga == '5%'){
                        iva5 = artC.precArtCarga * artC.cantArtCarga;
                        total5 = parseInt(total5) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                    }else{
                        exenta = artC.precArtCarga * artC.cantArtCarga;
                        totalExenta = parseInt(totalExenta) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                    }
                }
                totalVenta = totalVenta + (artC.precArtCarga * artC.cantArtCarga);
    
                contensBody += `
                <tr id="filaCarga_${id}">
                    <td class="border" id="pos_${id}">${id}</td>
                    <td class="border" id="nomArtCarga_${id}">${artC.nomArtCarga}</td>
                    <td class="text-end border" id="cantArtCarga_${id}">${artC.cantArtCarga}</td>
                    <td class="text-end border" id="preArtCarga_${id}">${artC.precArtCarga.toLocaleString()}</td>
                    <td class="text-end border" id="exentaArtCarga_${id}">${exenta.toLocaleString()}</td>
                    <td class="text-end border" id="iva5ArtCarga_${id}">${iva5.toLocaleString()}</td>
                    <td class="text-end border" id="iva10ArtCarga_${id}">${iva10.toLocaleString()}</td>
                    <td class="centered"></td>
                </tr>`;
            });
        }else{
            artC.forEach((artC, index) => {
                let id = index + 1;
                let iva10 = '', iva5 = '', exenta = '';
                parseInt(total5, total10, totalExenta);
                if(artC.ivaArtCarga == '10%'){
                    iva10 = artC.precArtCarga * artC.cantArtCarga;
                    total10 = parseInt(total10) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                }else{
                    if(artC.ivaArtCarga == '5%'){
                        iva5 = artC.precArtCarga * artC.cantArtCarga;
                        total5 = parseInt(total5) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                    }else{
                        exenta = artC.precArtCarga * artC.cantArtCarga;
                        totalExenta = parseInt(totalExenta) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                    }
                }
                totalVenta = totalVenta + (artC.precArtCarga * artC.cantArtCarga);
                
                contensBody += `
                <tr id="filaCarga_${id}">
                    <td class="border" id="pos_${id}">${id}</td>
                    <td class="border" id="nomArtCarga_${id}">${artC.nomArtCarga}</td>
                    <td class="text-end border" id="cantArtCarga_${id}">${artC.cantArtCarga}</td>
                    <td class="text-end border" id="preArtCarga_${id}">${artC.precArtCarga.toLocaleString()}</td>
                    <td class="text-end border" id="exentaArtCarga_${id}">${exenta.toLocaleString()}</td>
                    <td class="text-end border" id="iva5ArtCarga_${id}">${iva5.toLocaleString()}</td>
                    <td class="text-end border" id="iva10ArtCarga_${id}">${iva10.toLocaleString()}</td>
                    <td class="centered">
                        <button onclick="eliminarFilaCarga(${index});" class="btn btn-danger bi bi-trash3"></button>
                    </td>
                </tr>`;
            });
        }
        totalVentaCarga = totalVenta;

        tablaCargaBody.innerHTML = contensBody;
        
        let contentFoot = `
            <tr>
                <td class="text-start border" colspan="4">Sub Total</td>
                <td class="text-end border">${totalExenta.toLocaleString()}</td>
                <td class="text-end border">${total5.toLocaleString()}</td>
                <td class="text-end border">${total10.toLocaleString()}</td>
                <td class="centered border"></td>
            </tr>
        `;
        tablaCargaFoot.innerHTML = contentFoot;

        let contentFoot2 = `
            <tr>
                <td class="text-start border border-2 border-black fs-5" colspan="5">Total a pagar</td>
                <td class="text-end border border-2 border-black fs-5" colspan="3" id="subTotalCargado">${totalVenta.toLocaleString()}</td>
            </tr>
        `;
        tablaCargaFoot.insertAdjacentHTML('beforeend', contentFoot2);
        let contentFoot3 = `
            <tr>
                <td class="text-start border border-black" colspan="2">Liquidacion de IVA</td>
                <td class="text-end border border-black" colspan="2">${calcularIva5(total5).toLocaleString()}</td>
                <td class="text-end border border-black" colspan="2">${calcularIva10(total10).toLocaleString()}</td>
                <td class="text-end border border-black" colspan="2">${(calcularIva10(total10) + calcularIva5(total5)).toLocaleString()}</td>
            </tr>
        `;

        tablaCargaFoot.insertAdjacentHTML('beforeend', contentFoot3);
    } catch (ex) {
        console.log('No se pudo cargar los articulos');
    }
};
function buscarArticulo(){
    let pos_art = -1;
    let subTotal;
    let codBarrasArt = document.getElementById('codigoArticulo').value;
    let cantArticulo = document.getElementById('cantidadArticulo').value;
    pos_art = buscarPosArticulo(codBarrasArt);
    
    if(pos_art == -1){
        document.getElementById('nombreArticulo').value='Articulo no encontrado';
        document.getElementById('precioUnitario').value = '';
        document.getElementById('subTotal').value = '';   
    }else{
        subTotal = parseInt(cantArticulo) * articulos[pos_art].precio_art;
        document.getElementById('nombreArticulo').value = articulos[pos_art].nombre_art;
        document.getElementById('precioUnitario').value = (articulos[pos_art].precio_art).toLocaleString();        
        document.getElementById('subTotal').value = subTotal.toLocaleString();        
    }
}
function buscarPosArticulo(cod){
    let pos_art = -1;
    for(let x = 0; x < articulos.length; x++){
        if(cod == articulos[x].codBarras_art){
            pos_art = x;
            break;
        }
    }
    return pos_art;
}
function buscarPosCodArticulo(cod){
    let pos_art = -1;
    for(let x = 0; x < articulos.length; x++){
        if(cod == articulos[x].codigo_art){
            pos_art = x;
            break;
        }
    }
    return pos_art;
}
async function eliminarFilaCarga(pos){
    if(ventaGuardada == false){
        // Esperar a que el usuario ingrese la contraseña
        const { value: password } = await Swal.fire({
            title: "Ingrese contraseña de Autorizacion",
            input: "password",
            inputLabel: "Contraseña por defecto 123",
            inputPlaceholder: "",
            inputAttributes: {
                maxlength: "10",
                autocapitalize: "off",
                autocorrect: "off"
            }
        });
    
        // Verificar si se ingresó una contraseña
        if (password) {
            // Comparar la contraseña ingresada con la contraseña de autorización
            if (password == contAutorizacion) {
                
                artC.splice(pos, 1);
                tablaArtCargada();
    
            } else {
                // Mostrar un mensaje de error si la contraseña es incorrecta
                Swal.fire("Error", "Contraseña Incorrecta   ", "error");
            }
        } else {
            // Mostrar un mensaje si el usuario no ingresó ninguna contraseña
            Swal.fire("", "Cancelado", "info");
        }
    }
    
}




// Funciones de Fecha ----------------------------------------------------------------------------------------------------
function formatearFecha(datetimeLocal) {
    let fechaHora = new Date(datetimeLocal);

    let dia = ('0' + fechaHora.getDate()).slice(-2); // Añade un cero inicial si es necesario
    let mes = ('0' + (fechaHora.getMonth() + 1)).slice(-2); // Los meses comienzan desde 0, por lo que se añade 1
    let año = fechaHora.getFullYear();
    let horas = ('0' + fechaHora.getHours()).slice(-2);
    let minutos = ('0' + fechaHora.getMinutes()).slice(-2);

    return `${dia}/${mes}/${año} ${horas}:${minutos}`;
}
function fechaActual() {
    const ahora = new Date();
    
    const año = ahora.getFullYear();
    const mes = ('0' + (ahora.getMonth() + 1)).slice(-2); // Meses empiezan desde 0
    const día = ('0' + ahora.getDate()).slice(-2);

    return `${año}-${mes}-${día}`;
}
function formatearFechaDT(datetimeLocal) {
    let fechaHora = new Date(datetimeLocal);

    let dia = ('0' + fechaHora.getDate()).slice(-2); // Añade un cero inicial si es necesario
    let mes = ('0' + (fechaHora.getMonth() + 1)).slice(-2); // Los meses comienzan desde 0, por lo que se añade 1
    let año = fechaHora.getFullYear();
    let horas = ('0' + fechaHora.getHours()).slice(-2);
    let minutos = ('0' + fechaHora.getMinutes()).slice(-2);

    return `${dia}/${mes}/${año} ${horas}:${minutos}`;
}
function fechaHoraActual(id) {
    var inputFecha = document.getElementById(id);
    var fechaActual = new Date();

    // Obtener la fecha en formato YYYY-MM-DD
    var year = fechaActual.getFullYear();
    var month = (fechaActual.getMonth() + 1).toString().padStart(2, '0');
    var day = fechaActual.getDate().toString().padStart(2, '0');

    // Obtener la hora en formato HH:MM
    var hours = fechaActual.getHours().toString().padStart(2, '0');
    var minutes = fechaActual.getMinutes().toString().padStart(2, '0');

    // Formatear la fecha y hora como YYYY-MM-DDTHH:MM
    var fechaHora = `${year}-${month}-${day}T${hours}:${minutes}`;

    // Establecer el valor del input
    inputFecha.value = fechaHora;
}
function fechaHoraActualReturn() {
    var fechaActual = new Date();

    // Obtener la fecha en formato YYYY-MM-DD
    var year = fechaActual.getFullYear();
    var month = (fechaActual.getMonth() + 1).toString().padStart(2, '0');
    var day = fechaActual.getDate().toString().padStart(2, '0');

    // Obtener la hora en formato HH:MM
    var hours = fechaActual.getHours().toString().padStart(2, '0');
    var minutes = fechaActual.getMinutes().toString().padStart(2, '0');

    // Formatear la fecha y hora como YYYY-MM-DDTHH:MM
    var fechaHora = `${year}-${month}-${day}T${hours}:${minutes}`;

    // Establecer el valor del input
    return fechaHora;
}
function fechaActual_5dias() {
    const ahora = new Date();

    // Restar 5 días a la fecha actual
    ahora.setDate(ahora.getDate() - 5);

    const año = ahora.getFullYear();
    const mes = ('0' + (ahora.getMonth() + 1)).slice(-2); // Meses empiezan desde 0
    const día = ('0' + ahora.getDate()).slice(-2);

    return `${año}-${mes}-${día}`;
}




// Seccion de usuario ------------------------------------------------------------------------------------------------------
function buscarPosNomUsuario(nom){
    let pos = -1;
    usuarios.forEach((usuarios, index) => {
        if(usuarios.usuario == nom){
            pos = index;
        }
    });
    return pos;
};
function cargarDatosUsuario(){
    document.getElementById('usuarioLogeadoNavbar').textContent = usuLogeado.usuario;
    document.getElementById('nombreUsuario').textContent = usuLogeado.usuario;
    document.getElementById('nombreApellidoPerfil').textContent = usuLogeado.nombre + ' ' + usuLogeado.apellido;
    document.getElementById('correoPerfil').textContent = usuLogeado.correo;
    document.getElementById('telefonoPerfil').textContent = usuLogeado.telefono;
};




// Seccion de Ventas -------------------------------------------------------------------------------------------------------
function nuevo(){
    // Resetea todos los campos
    desabilitarCamposVentas(false);
    document.getElementById('formCliente').reset();
    document.getElementById('formCargarArticulo').reset();
    //resetea los totales y los articulos cargados
    totalVenta = 0;
    total10 = 0;
    total5 = 0;
    totalExenta = 0;
    ivaTotal5 = 0;
    ivaTotal10 = 0;
    totalIva = 0;
    artC = [];
    
    tablaArtCargada();

    document.getElementById('fechaA').value = fechaActual();
    document.getElementById('codCliente').focus();
    document.getElementById('codVenta').value = codSigVenta();
    let posTimbrado = timbradoHabilitado();
    document.getElementById('numTimbrado').value = timbrados[posTimbrado].numTimbrado;
    document.getElementById('iniVigencia').value = timbrados[posTimbrado].inicioVigencia;
    document.getElementById('finVigencia').value = timbrados[posTimbrado].finVigencia;

    tempNumSucursal = timbrados[posTimbrado].numSucursal;  //Se guarda en variables temporales para poder guardar despues por partes
    tempNumCaja = timbrados[posTimbrado].numCaja;
    tempNumFac = sigNumFactura();    

    let numFactura = tempNumSucursal + '-' + tempNumCaja + '-' + tempNumFac;
    document.getElementById('numFactura').textContent = numFactura;
    document.getElementById('usuarioCreacionVenta').textContent = '';
    document.getElementById('fechaCreacionVenta').textContent = '';
    document.getElementById('btnAnular').hidden = true;

    ventaGuardada = false;
    verifivarPlanilla();
}
//esta funcion verifica que la planilla a ser eliminada no esta en uso en alguna venta realizada
function verificarPlanillasVentas(id){
    let numPlanillaVerificacion = document.getElementById(id).value;
    let enUso = false;

    ventas.forEach((ventas, index) => {
        if(ventas.numPlanilla == numPlanillaVerificacion){
            enUso = true;
        }
    })
    return enUso;
}
//verifica si la planilla tiene alguna venta registrada y a la vez si esa venta le pertenece al usuario logeado
function verificarPlanillasVentasUsuario(id){
    let numPlanillaVerificacion = document.getElementById(id).value;
    let posPlanilla = buscarPosNumPlanilla(numPlanillaVerificacion);
    let enUso = false;

    ventas.forEach((ventas, index) => {
        if(ventas.numPlanilla == numPlanillaVerificacion && ventas.usuarioCreacion == planillas[posPlanilla].cajero || usuLogeado.rol == 'Admin'){
            enUso = true;
        }
    })
    return enUso;
}

function verificarNumFactura(){
    let correcto = false;
    let posTimbrado = buscarPosTimbrado(document.getElementById('numTimbrado').value);
    let num = sigNumFactura();
    if(posTimbrado !== -1){
        if(num >= parseInt(timbrados[posTimbrado].inicioNumFactura)  && num <= parseInt(timbrados[posTimbrado].finNumFactura)){
            console.log('Si esta dentro de los parametros');
            correcto = true;
        }else{
            console.log('Numero de factura no habilitado');
        }
    }else{
        Swal.fire({
            title: 'Timbrado Invalido',
            text: 'El numero de timbrado no se encuantra registrado',
            icon: 'error',
            timer: 2000
        })
    }
    return correcto;
}



function finalizarVenta(event){
    event.preventDefault();
    if(ventaGuardada == false){
        if(artC.length >= 1){
            if(document.getElementById('numPlanilla').value !== ''){
                let posPlanilla = buscarPosNumPlanilla(document.getElementById('numPlanilla').value);
                if(posPlanilla !== -1){
                    if(planillas[posPlanilla].cajero == usuLogeado.usuario || usuLogeado.rol == 'Admin'){
                        if(planillas[posPlanilla].fechaHoraCierre == ''){
                            if(verificarNumFactura() == true){
                                document.getElementById('montoTotalVenta').textContent = totalVentaCarga.toLocaleString();
                                let nuevaFila = `
                                <tr class="centered border" id="filaTipoValor_1">
                                    <td class="text-center">
                                        <select class="form-control" name="" id="selecTipoValor_1"></select>
                                    </td>
                                    <td class="text-end">
                                        <input class="form-control text-end" type="number" id="tipoValorMonto_1">
                                    </td>
                                    <td class="text-center">
                                        <button onclick="eliminarFilaTipoValor(1)" class="btn btn-danger"><i class="bi bi-x-circle-fill"></i></button>
                                    </td>
                                </tr>`;
                                
                                tablaFinalizarVentaBody.innerHTML = nuevaFila;
                                llenarSelectTiposValor(1);
                                document.getElementById('tipoValorMonto_1').value = totalVentaCarga;
                                $('#modalProcesarVenta').modal('show');
                            }else{
                                Swal.fire({
                                    title: 'Numero de factura no habilitado',
                                    text: 'El numero de factura no esta habilitada en el campo de timbrados',
                                    icon: 'info',
                                    showConfirmButton: true
                                })
                            }

                        }else{
                            Swal.fire({
                                position: "center",
                                icon: "info",
                                title: "La planilla ya está cerrada",
                                showConfirmButton: true,
                                timer: 3000
                            })
                        }
                    }else{
                        Swal.fire({
                            position: "center",
                            icon: "info",
                            title: "La planilla no pertenece al cajero actual",
                            showConfirmButton: true,
                            timer: 3000
                        })   
                    }
                }else{
                    Swal.fire({
                        position: "center",
                        icon: "info",
                        title: "Numero de planilla no encontrada",
                        showConfirmButton: true,
                        timer: 3000
                    }) 
                }
            }else{
                Swal.fire({
                    position: "center",
                    icon: "info",
                    title: "Ingresa el numero de planilla",
                    showConfirmButton: true,
                    timer: 3000
                }) 
            }            
        }else{   
            if(!document.getElementById('formCargarArticulo').checkValidity()){
                document.getElementById('codigoArticulo').focus();
            }
        }
    }
}
function procesarVenta(){
    let tabla = document.getElementById('tablaFinalizarVentaBody');
    if(tabla.rows.length >= 1){
        let metodoPago = [];  
        let montoMetodoPago = [];  
        let sumMontoMetodoPago = 0;
        for(let x = 1; x <= tabla.rows.length; x++){
            metodoPago.push(document.getElementById('selecTipoValor_' + x).value);
            montoMetodoPago.push(parseInt(document.getElementById('tipoValorMonto_' + x).value));
            sumMontoMetodoPago += parseInt(document.getElementById('tipoValorMonto_' + x).value);
        }
        // Si el total en metodos de pago coincide se guarda la venta
        if(sumMontoMetodoPago == totalVentaCarga){
            let codBArt = [];
            let codArticulo = [];
            let preUnit = [];
            let cantArticulo = [];
            let tipoIvaArt = [];
            let condVenta = 'Contado';
            
            for(let x = 0; x < document.getElementById('tablaCargaBody').rows.length; x++){
                let posArt = buscarPosArticulo(artC[x].codArtCarga);
                codBArt[x] = articulos[posArt].codBarras_art;
                codArticulo[x] = artC[x].codArtCarga;
                preUnit[x] = artC[x].precArtCarga;
                cantArticulo[x] = artC[x].cantArtCarga;
                tipoIvaArt[x] = artC[x].ivaArtCarga;

                // Tiene que restar la cantidad vendida del stock
                articulos[posArt].stock_art -= artC[x].cantArtCarga;

            }
            if(document.getElementById('credito').checked == true){
                condVenta = 'Credito';
            }
            let codigoC = parseInt(document.getElementById('codCliente').value);
            if(buscarPosCliente(codigoC) == -1){
                codigoC = 0;
            }
            let nuevaVenta = {
                codVenta: parseInt(document.getElementById('codVenta').value),
                codCliente:codigoC, 
                fechaVenta: document.getElementById('fechaA').value,
                codArt: codArticulo,
                codBarrasArt: codBArt,
                precioUnit: preUnit,
                cantArt: cantArticulo,
                tipoIva: tipoIvaArt ,
                numTimbrado: document.getElementById('numTimbrado').value,         
                numSucursal: tempNumSucursal,
                numCaja: tempNumCaja,
                numFactura: tempNumFac,
                nombreRazonSocial: document.getElementById('nomRazonCliete').value,
                ruc: document.getElementById('rucCliente').value,
                condicionVenta: condVenta,
                metodoPago: metodoPago,
                montoMetodoPago: montoMetodoPago,
                numPlanilla: parseInt(document.getElementById('numPlanilla').value),
                estadoVenta: true,
                estadoFactura: true,
                fechaHoraCreacion: fechaHoraActualReturn(),
                usuarioCreacion: usuLogeado.usuario
            }
            
            ventas.push(nuevaVenta);    // Actualiza ventas local
            localStorage.setItem('lSVentas', JSON.stringify(ventas));       // Actualiza ventas de localstorage
            localStorage.setItem('lSArticulos', JSON.stringify(articulos));       // Actualiza los articulos del localstorage
            ventaGuardada = true;
            if(usuLogeado.rol == 'Admin'){
                document.getElementById('btnAnular').hidden = false;
            }
            Swal.fire({
                position: "center",
                icon: "success",
                title: "Exito",
                text: "Venta Guardada",
                showConfirmButton: true,
                timer: 2000
            })
            $('#modalProcesarVenta').modal('hide');
        }else{
            Swal.fire({
                title: "Error",
                text: "El monto no coincide con el total de Venta",
                icon: "error",
                timer: 1500
            })
        }
   }else{
    Swal.fire({
        title: "Error",
        text: "Inserta el tipo de valor",
        icon: "error",
        timer: 1500
    })
   }
}
function anularVenta(){
    Swal.fire({
        title: "Eliminar Venta?",
        text: "No se puede revertir!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: "Si",
        cancelButtonText: "No"
        }).then((result) => {
        if (result.isConfirmed) {
            let codVenta = document.getElementById('codVenta').value;
            let posVenta = buscarPosVentas(codVenta);
            ventas[posVenta].estadoVenta = false;
            ventas[posVenta].estadoFactura = false;

            // Tiene que volver a sumar a stock los articulos de la venta anulada
            for(let x = 0; x < ventas[posVenta].codArt.length; x++){
                let pos = buscarPosCodArticulo(ventas[posVenta].codArt[x]);
                articulos[pos].stock_art += ventas[posVenta].cantArt[x];
            }
            // Se guarda como venta anulada sin anular la factura fisicamente
            localStorage.setItem('lSVentas', JSON.stringify(ventas));
            localStorage.setItem('lSArticulos', JSON.stringify(articulos));

            Swal.fire({
                title: "Venta Eliminada!",
                icon: "success"
            });
            Swal.fire({
                title: "Anular Factura Fisicamente?",
                text: "No se puede revertir!",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                confirmButtonText: "Si",
                cancelButtonText: "No"
            }).then((result) => {
            if (result.isConfirmed) {

                ventas[posVenta].estadoFactura = true;
                nuevo();
                Swal.fire({
                    title: "Anulacion Exitosa!",
                    text: "Factura Fisicamente Anulada",
                    icon: "success"
                })
            }            
            })
        }
    })
}
function ventaAnterior(num){
    try{
        let codVenta = document.getElementById('codVenta').value;
        if(codVenta == 1 || codVenta - num == 0){
        }else{
            let posVenta = buscarPosVentas(codVenta - num);
            if(ventas[posVenta].usuarioCreacion == usuLogeado.usuario || usuLogeado.rol == 'Admin'){
               if(ventas[posVenta].estadoVenta == true){
                   mostrarVenta(posVenta);
               }else{
                   ventaAnterior(num + 1);
               }
            }else{
                ventaAnterior(num + 1);
            }
        } 
    }catch(error){
        console.log(error);
    }
}
function ventaPosterior(num){
    try{        
        let codVenta = parseInt(document.getElementById('codVenta').value) + num;
        let posVenta = buscarPosVentas(codVenta);
        if(posVenta == -1 || posVenta >= ventas.length){
        }else{
            posVenta = posVenta;
            if(ventas[posVenta].usuarioCreacion == usuLogeado.usuario || usuLogeado.rol == 'Admin'){
                if(ventas[posVenta].estadoVenta == true){
                    mostrarVenta(posVenta);
                }else{
                    ventaPosterior(num + 1);
                }
            }else{
                ventaPosterior(num + 1);
            }
        }
    }catch(error){
        console.log(error);
    }
}
function mostrarVenta(pos){
    ventaGuardada = true;
    desabilitarCamposVentas(true);
    document.getElementById('codVenta').value = ventas[pos].codVenta;
    document.getElementById('fechaA').value = ventas[pos].fechaVenta;
    document.getElementById('codCliente').value = ventas[pos].codCliente;
    buscarCliente();

    if(ventas[pos].condicionVenta == 'Contado'){
        document.getElementById('contado').checked = true;  
    }else{
        document.getElementById('credito').checked = true;  
    }
    document.getElementById('numPlanilla').value = ventas[pos].numPlanilla;

    let timbrado = ventas[pos].numTimbrado;
    let posTimbrado = buscarPosTimbrado(timbrado);
    document.getElementById('numTimbrado').value = timbrado;
    document.getElementById('iniVigencia').value = timbrados[posTimbrado].inicioVigencia;
    document.getElementById('finVigencia').value = timbrados[posTimbrado].finVigencia;
    let nSucursal = ventas[pos].numSucursal;
    let nCaja = ventas[pos].numCaja;
    let nFactura = ventas[pos].numFactura;

    document.getElementById('numFactura').textContent = nSucursal+'-'+nCaja+'-'+nFactura;
    document.getElementById('usuarioCreacionVenta').textContent = ventas[pos].usuarioCreacion + '  ';
    document.getElementById('fechaCreacionVenta').textContent = formatearFechaDT(ventas[pos].fechaHoraCreacion);

    artC = [];

    for(let x = 0; x < ventas[pos].codArt.length; x++){
        let posArt = buscarPosArticulo(ventas[pos].codArt[x]);
        let registroCarga = {
            codArtCarga: ventas[pos].codArt[x],
            nomArtCarga: articulos[posArt].nombre_art,
            cantArtCarga: ventas[pos].cantArt[x],
            precArtCarga: ventas[pos].precioUnit[x],
            ivaArtCarga: ventas[pos].tipoIva[x]
        }
        artC.push(registroCarga);
    }
    if(usuLogeado.rol == 'Admin'){
        document.getElementById('btnAnular').hidden = false;
    }
    tablaArtCargada();    
}
function buscarPosVentas(cod){
    let pos = -1;
    for(let x=0; x < ventas.length; x++){
        if(cod == ventas[x].codVenta){
            pos = x;
            break;
        }
    }
    return pos;
}
function desabilitarCamposVentas(valor){
    document.getElementById('codCliente').disabled = valor;
    document.getElementById('buscarCliente').disabled = valor;
    document.getElementById('rucCliente').disabled = valor;
    document.getElementById('nomRazonCliete').disabled = valor;
    document.getElementById('contado').disabled = valor;
    document.getElementById('credito').disabled = valor;
    document.getElementById('numPlanilla').disabled = valor;
    document.getElementById('btnNumPlanilla').disabled = valor;
    document.getElementById('codigoArticulo').disabled = valor;
    document.getElementById('cantidadArticulo').disabled = valor;
    document.getElementById('agregarArticulo').disabled = valor;
    document.getElementById('btnGuardar').disabled = valor;
}
function calcularIva5(valor){
    let iva = Math.ceil(valor / 21);
    return iva;
}
function calcularIva10(valor){
    let iva = Math.ceil(valor / 11);
    return iva;
}
function codSigVenta(){
    let cod;
    for(let i = 0; i < ventas.length; i++){
        if(i == 0){
            cod = ventas[i].codVenta;
        }else{
            if(ventas[i].codVenta > cod){
                cod = ventas[i].codVenta;
            }
        }
    }
    return cod + 1;
}
function añadirFilaTipoValor(){
    let filas = tablaFinalizarVentaBody.rows.length;
    if(filas < 5){
        id = filas + 1;
        let nuevaFila = `
        <tr class="centered border" id="filaTipoValor_${id}">
            <td class="text-center">
                <select class="form-control" name="" id="selecTipoValor_${id}"></select>
            </td>
            <td class="text-end">
                <input class="form-control text-end" type="number" id="tipoValorMonto_${id}">
            </td>
            <td class="text-center">
                <button onclick="eliminarFilaTipoValor(${id})" class="btn btn-danger"><i class="bi bi-x-circle-fill"></i></button>
            </td>
        </tr>`;

        tablaFinalizarVentaBody.insertAdjacentHTML('beforeend', nuevaFila);
        llenarSelectTiposValor(id);

    }else{
        Swal.fire({
            title: "No se pueden agregar mas filas!",
            text: "Solo se permiten 5 metodos de pago por venta",
            icon: "error",
            timer: 1500
        })
    }
}
function eliminarFilaTipoValor(id){
    var fila = document.getElementById('filaTipoValor_' + id);
    fila.remove();
}
function llenarSelectTiposValor(id) {
    var select = document.getElementById('selecTipoValor_' + id);
    if(select.options.length < 1){
        tiposValor.forEach((tiposValor, index) => {
            var option = document.createElement("option");
            option.text = tiposValor.valor;
            option.value = tiposValor.valor;
            select.appendChild(option);
        })

    }
};
function buscarPosTimbrado(num){
    let pos = -1;
    timbrados.forEach((timbrados, index) => {
        if(timbrados.numTimbrado == num){
            pos = index;
        }
    });
    return pos;
}
function timbradoHabilitado(){
    let pos;
    let fecha = new Date(fechaActual()); // Convertir la fecha actual a objeto Date para comparación
    for (let x = 0; x < timbrados.length; x++) {
        let inicio = new Date(timbrados[x].inicioVigencia);
        let fin = new Date(timbrados[x].finVigencia);
        if (fecha >= inicio && fecha <= fin) {
            pos = x;
            break;
        }
    }
    return pos;
}
function sigNumFactura(){
    let mayor = 0;
    ventas.forEach((ventas, index) => {
        if(parseInt(ventas.numFactura) > mayor && ventas.estadoFactura == true){
            mayor = parseInt(ventas.numFactura);
        }
    })

    // Sumar 1 al número
    mayor += 1;
    
    // Convertir el número de vuelta a una cadena y rellenar con ceros a la izquierda
    return mayor.toString().padStart(7, '0');
}
function imprimirFactura(){
    let codVenta = document.getElementById('codVenta').value;

    if(ventaGuardada == true){
        localStorage.setItem('lSFactura', codVenta);
        const opciones = 'width=1500,height=800,scrollbars=yes,resizable=yes';
        window.open('factura.html', 'popup', opciones);
    }else{
        Swal.fire({
            position: "center",
            icon: "info",
            title: "Debes guardar para imprimir",
            showConfirmButton: true,
            timer: 1500
        })
    }
}



// Secciones informe de venta----------------------------------------------------------------------------------------
const opcionesDataTableInfoVentas = {
    columnDefs: [
        {className: "centered border bg-primary-200 text-100", targets: [0, 1, 2, 3, 4, 5, 6, 7, 8]}
    ],
    destroy: true,  
    lengthMenu: [10, 20, 50, 100],
    dom: 'Bfrltip', 
    buttons: [
        {
            extend: 'excelHtml5',
            text: '<i class="bi bi-file-earmark-excel-fill"></i>',
            titleAttr: 'Exportar a Excel',
            className: 'btn btn-success'
        },
        {
            extend: 'pdfHtml5',
            text: '<i class="bi bi-file-earmark-pdf-fill"></i>',
            titleAttr: 'Exportar a PDF',
            title: 'Reporte de Ventas',
            className: 'btn btn-danger',
            messageTop: '',
            messageBottom: '',
            customize: function (doc) {
                // Agregar logo en la cabecera
                doc.content.splice(0, 0, {
                    margin: [0, 0, 0, 12],
                    alignment: 'center',
                    image: 'data:image/jpeg;base64,',  // Reemplaza con tu imagen en base64
                });

                doc.footer = function(currentPage, pageCount) {
                    return {
                        text: currentPage.toString() + ' de ' + pageCount,
                        alignment: 'center',
                        style: 'footerStyle'
                    };
                };
                doc.styles.footerStyle = {
                    fontSize: 10,
                    margin: [0, 0, 0, 10]
                };
            }
        },
        {
            extend: 'print',
            text: '<i class="bi bi-printer-fill"></i>',
            titleAttr: 'Imprimir',
            className: 'btn btn-info'
        }
    ],
    language: {
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};
const mostrarDatosInfoVentas = async () => {
    try {        
        let contensBody = ``;
        let numPlanilla = document.getElementById('numPlanilla').value;

        document.getElementById('numPlanillaInforme').textContent = numPlanilla + ' - CAJERO - ' + planillas[buscarPosNumPlanilla(numPlanilla)].cajero;
        infoVentas.forEach((infoVentas, index) => {
            let id = index + 1;
            let codCliente = infoVentas.codCliente;
            let posCliente = buscarPosCliente(codCliente);
            let nomCliente = clientes[posCliente].apellidoCliente + ', ' + clientes[posCliente].nombreCliente;
            let numFactura = infoVentas.numSucursal + '-' + infoVentas.numCaja + '-' + infoVentas.numFactura;

            
            let totalGeneral = 0;
            for(let x = 0; x < infoVentas.codArt.length; x++){
                let total = infoVentas.cantArt[x] * infoVentas.precioUnit[x];
                totalGeneral = totalGeneral + total;
            }
            
            contensBody += `
            <tr>
                <td id="ordenVentas_${id}">${id}</td>
                <td id="codigoVenta_${id}">${infoVentas.codVenta}</td>
                <td id="clienteVentas${id}">${nomCliente}</td>
                <td id="clienteVentas${id}">${formatearFecha(infoVentas.fechaHoraCreacion)}</td>
                <td id="metodoPagoVentas${id}">${infoVentas.metodoPago}</td>
                <td id="totalVentas${id}">${totalGeneral.toLocaleString()}</td>
                <td id="numFacturaVentas${id}">${numFactura}</td>
                <td id="cajeroVentas${id}">${infoVentas.usuarioCreacion}</td>
                <td>
                    <button id="btnAccionInfoVenta" onclick="verVenta(codigoVenta_${id})" class="btn btn-success "><i class="bi bi-eye-fill"></i></button></td>
            </tr>`;
        });
        tablaInformeVentasBody.innerHTML = contensBody;
    } catch (ex) {
        alert(ex);
        console.log('No se pudo cargar la planilla');
    }
};
function verVenta(id){
    var modalVentas = bootstrap.Modal.getInstance(document.getElementById('modalInformeVentas')); // Obtener la instancia del modal de Bootstrap
    let codVenta = id.textContent;
    let posVenta = buscarPosVentas(codVenta);
    mostrarVenta(posVenta);
    $('#modalInformeVentas').modal('hide');    
}
const informeVentas = async() => {
    if(verificarPlanillasVentasUsuario('numPlanilla') == true){
        let nPlanilla = document.getElementById('numPlanilla').value;
        if(nPlanilla !== ''){
            let posPlanilla = buscarPosNumPlanilla(nPlanilla);
            if(posPlanilla !== -1){
                if(planillas[posPlanilla].cajero == usuLogeado.usuario || usuLogeado.rol == 'Admin'){
                    infoVentas = [];
                    ventas.forEach((ventas, index) =>{
                        if(ventas.numPlanilla == nPlanilla && ventas.estadoVenta == true){
                            if(ventas.usuarioCreacion == planillas[posPlanilla].cajero || usuLogeado.rol == 'Admin'){
                                infoVentas.push(ventas);
                            }
                        }
                    })
                    await tablaInfoVentasCargada();  
                    $('#modalInformeVentas').modal('show');          
                }            
            }
        }
    }
}











// Seccion de tipos de Valor-----------------------------------------------------------------------------------------------
let nuevotipoValor = false;
let editarFilaTipoValor = false;
let idFilaTipoValorHabilitada;

const tablaTipoValorCargada = async()=>{
    if(tablaTipoValorIniciada){
        dataTableTipoValor.destroy();
    }
    await mostrarDatosTiposValor();
    dataTableTipoValor = $("#tablaTipoValor").DataTable(opcionesDataTableTipoValor);
    tablaTipoValorIniciada = true;
};

const opcionesDataTableTipoValor = {
    destroy: true,  // Para que la tabla sea destruible
    dom: '', //Para que 
    language:{
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};
const mostrarDatosTiposValor = async () => {
    try {        
        let contensBody = ``;
        tiposValor.forEach((tiposValor, index) => {
            let id = index + 1;

            contensBody += `
            <tr id="filaTiposValor_${id}">
                <td><input type="text" class="form-control text-center" id="codigoTipoValor_${id}" value="${tiposValor.codValor}" disabled></td>
                <td><input type="text" class="form-control text-center" id="tipoValor_${id}" value="${tiposValor.valor}" disabled></td>
                <td><input type="datetime-local" class="form-control text-center" id="fechaCreacion_${id}" value="${tiposValor.fechaCreacion}" disabled></td>
                <td><input type="text" class="form-control text-center" id="usuarioCreacion_${id}" value="${tiposValor.usuarioCreacion}" disabled></td>
                <td>
                    <button id="btnEditarFilaTipoValor_${id}" onclick="modificarFilaTipoValor(${id})" class="btn btn-success bi bi-pencil-square"></button>
                    <button id="btnEliminarFilaTipoValor_${id}" onclick="eliminarFilaTipoValor(${id});" class="btn btn-danger bi bi-trash3"></button>
                </td>
            </tr>`;
        });

        tablaTiposValorBody.innerHTML = contensBody;
    } catch (ex) {
        alert(ex);
        console.log('No se pudo cargar los tipos de valor');
    }
};

function modificarFilaTipoValor(id){
    if(nuevotipoValor == false){
        habilitarFilaTipoValor(id);
    }
}

function habilitarFilaTipoValor(id) {
    idFilaTipoValorHabilitada = id;
    let cantidadFila = document.getElementById('tablaTiposValorBody').rows.length;

    document.getElementById('codigoTipoValor_' + id).disabled = false;
    document.getElementById('tipoValor_' + id).disabled = false;
    document.getElementById('btnEditarFilaTipoValor_' + id).hidden = true;
    document.getElementById('btnEliminarFilaTipoValor_' + id).hidden = true;
    document.getElementById('btnGuardarTipoValor').hidden = false;
    document.getElementById('btnNuevoTipoValor').hidden = true;
    editarFilaTipoValor = true;

    for(let i = 1; i <= cantidadFila; i++){
        if(i !== id){
            document.getElementById('codigoTipoValor_' + i).disabled = true;
            document.getElementById('tipoValor_' + i).disabled = true;
            document.getElementById('btnEditarFilaTipoValor_' + i).hidden = false;
            document.getElementById('btnEliminarFilaTipoValor_' + i).hidden = true;
        }
    }

    document.getElementById('modalTiposValor').addEventListener('hidden.bs.modal', () => {
        editarFilaTipoValor = false;
        desabilitarFilaTipoValor(id);
        document.getElementById('btnGuardarTipoValor').hidden = true;
        document.getElementById('btnNuevoTipoValor').hidden = false;
    })

};
function desabilitarFilaTipoValor(id) {
    if(id){
        document.getElementById('codigoTipoValor_' + id).disabled = true;
        document.getElementById('tipoValor_' + id).disabled = true;
    }
};

function nuevaFilaTipoValor() {
    if(nuevotipoValor == false && editarFilaTipoValor == false){
        nuevotipoValor = true;
        try {
            // Se ocultan todos los botones
            for(let i = 1; i <= tablaTiposValorBody.rows.length; i++){
                    document.getElementById('btnEditarFilaTipoValor_' + i).hidden = true;
                    document.getElementById('btnEliminarFilaTipoValor_' + i).hidden = true;
            }

            document.getElementById('btnGuardarTipoValor').hidden = false;
            document.getElementById('btnNuevoTipoValor').hidden = true;
            let id = document.getElementById('tablaTiposValorBody').rows.length + 1;
            let codTipoValor = codigoMayorTipoValor() + 1;
            let nuevaFila = `
                <tr id="filaTiposValor_${id}">
                <td><input type="text" class="form-control text-center" id="codigoTipoValor_${id}" value="${codTipoValor}"></td>
                <td><input type="text" class="form-control text-center" maxlength="20" id="tipoValor_${id}" value=""></td>
                <td><input type="datetime-local" class="form-control text-center" id="fechaCreacion_${id}" hidden></td>
                <td><input type="text" class="form-control text-center" id="usuarioCreacion_${id}" hidden></td>
                <td>
                </td>
            </tr>`;

            // Agregar la nueva fila al cuerpo de la tabla sin reemplazar todo el contenido
            tablaTiposValorBody.insertAdjacentHTML('beforeend', nuevaFila);
            document.getElementById('tipoValor_' + id).focus(); 
            
            // Se cierra el modal Tipos Valor
            document.getElementById('modalTiposValor').addEventListener('hidden.bs.modal', () => {
                if(nuevotipoValor == true){
                    if (tablaTiposValorBody.rows.length > 1) tablaTiposValorBody.deleteRow(-1);
                    document.getElementById('btnGuardarTipoValor').hidden = true;
                    document.getElementById('btnNuevoTipoValor').hidden = false;
                    for(let i = 1; i <= tablaTiposValorBody.rows.length; i++){
                        document.getElementById('btnEditarFilaTipoValor_' + i).hidden = false;
                        document.getElementById('btnEliminarFilaTipoValor_' + i).hidden = false;
                    }
                }
                nuevotipoValor = false;
            });
        } catch (ex) {
            alert(ex);        
        }
    }
};

function codigoMayorTipoValor(){
    let may = 0;
    for(let x = 0; x < tiposValor.length; x++){
        if(x == 0){
            may = tiposValor[x].codValor;
        }else{
            if(tiposValor[x].codValor > may){
                may = tiposValor[x].codValor;
            }
        }
    }
    return may;
}

function cancelarNuevoTipoValor(){
    nuevotipoValor = false;
    $('#modalTiposValor').modal('hide');
}
function mostrarTiposValor(){
    tablaTipoValorCargada();
    $('#modalTiposValor').modal('show');
    $('#offcanvasDarkNavbar').offcanvas('hide');
}
function guardarTipoValor(){ 
    try{
        var modalBS = bootstrap.Modal.getInstance(document.getElementById('modalTiposValor')); // Obtener la instancia del modal de Bootstrap
        if(nuevotipoValor == true){
            // Se guarda como nuevo tipo valor
            let id = document.getElementById('tablaTiposValorBody').rows.length;
            
            let nuevaTValor ={
                codValor: parseInt(document.getElementById('codigoTipoValor_' + id).value),
                valor: document.getElementById('tipoValor_' + id).value,
                fechaCreacion: fechaHoraActualReturn(),
                usuarioCreacion: usuLogeado.usuario
            };

            tiposValor.push(nuevaTValor);
            localStorage.setItem('lSTiposValor', JSON.stringify(tiposValor));
            nuevotipoValor = false;
            Swal.fire({
                position: "center",
                icon: "success",
                title: "Tipo de Valor Guardado",
                showConfirmButton: true,
                timer: 1500
            })
            tablaTipoValorCargada();
        }else{
            // se guarda como tipo Valor modificada
            let idFila = idFilaTipoValorHabilitada;
            let codTipoValor = parseInt(document.getElementById('codigoTipoValor_' + idFila).value);
            let pos = buscarPosTipoValor(codTipoValor);

            tiposValor[pos].valor = document.getElementById('tipoValor_' + idFila).value;

            localStorage.setItem('lSTiposValor', JSON.stringify(tiposValor));
            tablaTipoValorCargada();                     
        }
    }catch(ex){
        alert(ex);
        console.log('No se pudo guardar');
    }
};
function eliminarFilaTipoValor(id){
    let pos = buscarPosTipoValor(parseInt(document.getElementById('codigoTipoValor_' + id).value));
    if(!verificarUsoTipoValor(pos)){
        Swal.fire({
            title: "Eliminar Tipo de Valor?",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#d33",
            cancelButtonColor: "#3085d6",
            confirmButtonText: "Si, Eliminar"
        }).then((result) => {
            if (result.isConfirmed) {
                tiposValor.splice(pos, 1);
                localStorage.setItem('lSTiposValor', JSON.stringify(tiposValor));
                tablaTipoValorCargada();
            }
        });
    }else{
        Swal.fire({
            title: 'No se puede Eliminar!',
            text: 'Este tipo de valor esta en uso',
            icon: 'error',
            timer: 1500
        })
    }

}

function buscarPosTipoValor(cod){
    let pos = -1;
    tiposValor.forEach((tiposValor, index) => {
        if(tiposValor.codValor === cod){
            pos = index;
        }
    })
    return pos;
}

function verificarUsoTipoValor(pos){
    let uso = false;
    for(let x = 0; x < ventas.length; x++){
        for(let i = 0; i < ventas[x].metodoPago.length; i++){
            if(ventas[x].metodoPago[i] == tiposValor[pos].valor){
                uso = true;
            }
        }
    }
    return uso;
}