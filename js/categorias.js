...
const tablaCargada = async()=>{
    ...
    dataTable = $("#tablaCategorias").DataTable(opcionesDataTable);
...
const mostrarDatos = async =>{
    try{        
        ...
        categorias.forEach((categorias, index) => {
            ...
            contensBody+= `
            <tr id="fila_` + id + `">
                <td id="codigo_` + id + `">${categorias.codigo}</td>
                <td id="categoria_` + id + `">${categorias.categoria}</td>
                <td id="estado_` + id + `">${categorias.estado}</td>
                <td id="fechaC_` + id + `">${formatearFechaDT(categorias.fechaCreacion)}</td>
                <td id="fechaM_` + id + `">${formatearFechaDT(categorias.fechaModificacion)}</td>
                <td>                
                <!-- los botones de editar y eliminar -->
                <button onclick="editarFilaCategorias(${id});" data-bs-toggle="modal" data-bs-target="#modalCategorias" class="btn btn-success bi bi-pencil-square"></button>
                <button onclick="eliminarFilaCategorias(${id});" class="btn btn-danger bi bi-trash3"></button>
                </td>
            </tr>`;
            tablaCategoriasBody.innerHTML = contensBody; 
        });
    }catch(ex){
        alert(ex);
...
function editarFilaCategorias(idFila){
    document.getElementById('labelAgregarModificar').textContent = "Modificar Categoria";
    var codCategoria = document.getElementById('codigo_' + idFila).textContent;
    var pos = buscarPosCategoria(codCategoria);
    document.getElementById('codCategoria').value = categorias[pos].codigo;
    document.getElementById('nomCategoria').value = categorias[pos].categoria;
    document.getElementById('fechaCCategoria').textContent = formatearFechaDT(categorias[pos].fechaCreacion);
    document.getElementById('fechaMCategoria').textContent = formatearFechaDT(categorias[pos].fechaModificacion);
    document.getElementById('usuarioCCategoria').textContent =categorias[pos].usuarioCreacion;
    document.getElementById('usuarioMCategoria').textContent = categorias[pos].usuarioModificacion;

    let selecEstado = document.getElementById('estCategoria');
    for(let x = 0; x < selecEstado.options.length; x++){
        if(selecEstado.options[x].value === categorias[pos].estado){
...
function buscarPosCategoria(cod){
    let pos = -1;
    for(let x = 0; x < categorias.length; x++){
        if(cod == categorias[x].codigo){
            pos = x;
            break;
        }
    }
...
async function guardarCategoria(event){ 
    event.preventDefault();
    let codigoCategoria = parseInt(document.getElementById('codCategoria').value);
    let modificacion = buscarPosCategoria(codigoCategoria);

    if(modificacion !== -1){   //Si encuentra que ese codigo de la categoria ya existe estonces se guarda como modifacado
            categorias[modificacion].categoria = document.getElementById('nomCategoria').value;
            categorias[modificacion].estado = document.getElementById('estCategoria').value;
            categorias[modificacion].fechaModificacion = fechaHoraActualReturn();
            categorias[modificacion].usuarioModificacion = usuLogeado.usuario;
    
            localStorage.setItem('lSCategorias', JSON.stringify(categorias));
            $('#modalCategorias').modal('hide');
    }else{  //Se guarda como nuevo articulo
        let nuevaCategoria = {
            codigo: parseInt(document.getElementById('codCategoria').value),
            categoria: document.getElementById('nomCategoria').value, 
            estado: document.getElementById('estCategoria').value, 
...
async function eliminarFilaCategorias(idFila){
    var fila = document.getElementById('fila_' + idFila);
    var codCategoria = document.getElementById('codigo_' + idFila).textContent;
    var pos = buscarPosCategoria(codCategoria);
    if(verificarUsoCategoria(categorias[pos].categoria) == false){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                categorias.splice(pos, 1);  //elimina el articulo en esa posicion
                localStorage.setItem('lSCategorias', JSON.stringify(categorias));
                // Eliminar la fila
                fila.remove();
                Swal.fire({
        ...
        Swal.fire({
            ...
            text: "Esta categoria esta en úso",
            icon: "error",
            showConfirmButton: true,
            timer: 2000
        })
...
function codCategoriaCarga(){
    document.getElementById('formCategorias').reset();
    document.getElementById('codCategoria').value = codigoMayor() + 1;
    document.getElementById('labelAgregarModificar').textContent = 'Agregar Categoria';
}
...
function codigoMayor(){
    ...
    for(let x = 0; x < categorias.length; x++){
        if(x == 0){
            may = categorias[x].codigo;
        }else{
            if(categorias[x].codigo > may){
                may = categorias[x].codigo;
            }
        }
    }
...
function verificarUsoCategoria(categoria){
    let uso = false;
    articulos.forEach((articulos, index) => {
        if(articulos.categoria_art == categoria){
            uso = true;
        }
    })
    return uso;
