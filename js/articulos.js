...
let personas = JSON.parse(localStorage.getItem("lSPersonas"));
...
let categorias = JSON.parse(localStorage.getItem("lSCategorias"));
    ...
    for (var i = 0; i < rowCount; i++) {
        doc.content[tableIndex].table.body[i].forEach(function (cell) {
            ...
            if (i === 0) {
                ...
                newStyle.color = '#ffffff'; // Color blanco para el texto del encabezado
...
const tablaCargada = async()=>{
    ...
    dataTable = $("#tablaArticulos").DataTable(opcionesDataTable);
...
const mostrarDatos = async =>{
    try{        
        ...
        articulos.forEach((articulos, index) => {
            let id = index + 1;
            contensBody+= `
            <tr class="filaSeleccionada border border-3 border-black" id="fila_${id}">
                <td class="centered border bg-primary-200 text-white" id="codigo_` + id + `">${articulos.codigo_art}</td>
                <td class="centered border bg-primary-200 text-white" id="nombre_` + id + `">${articulos.nombre_art}</td>
                <td class="centered border bg-primary-200 text-white" id="categoria_` + id + `">${articulos.categoria_art}</td>
                <td class="centered border bg-primary-200 text-white" id="proveedor_` + id + `">${articulos.proveedor_art}</td>
                <td style="vertical-align: middle;" class="text-end border bg-primary-200 text-white" id="precio_` + id + `">${(articulos.precio_art).toLocaleString()}</td>
                <td class="centered border bg-primary-200 text-white" id="precio_` + id + `">${(articulos.iva_art).toLocaleString()}</td>
                <td style="vertical-align: middle;" class="text-end border bg-primary-200 text-white" id="stock_` + id + `">${(articulos.stock_art.toLocaleString())}</td>
                <td class="centered border bg-primary-200 text-white" id="estado_` + id + `">${articulos.estado_art}</td>
                <td class="centered border bg-primary-200 text-white" id="fechaC_` + id + `">${formatearFechaDT(articulos.fecha_creacion_art)}</td>
                <td class="centered border bg-primary-200 text-white" id="fechaM_` + id + `">${articulos.usuarioCreacion}</td>
                <td class="centered border bg-primary-200 text-white">  
                    <button onclick="editarFilaArticulos(${id});" data-bs-toggle="modal" data-bs-target="#modalArticulos" class="btn btn-success bi bi-pencil-square"></button>
                    <button onclick="eliminarFilaArticulos(${id});" class="btn btn-danger bi bi-trash3"></button>
                </td>
            </tr>`;
        });
        tablaArticulosBody.innerHTML = contensBody; 
    }catch(ex){
        alert(ex);
...
function editarFilaArticulos(idFila){
    document.getElementById('labelAgregarModificar').textContent = "Modificar Articulo";
    llenarSelectProveedores();
    llenarSelectCategorias();
    var codArticulo = document.getElementById('codigo_' + idFila).textContent;
    var pos_art = buscarPosArticulo(codArticulo);
    
    document.getElementById('codArticulo').value = articulos[pos_art].codigo_art;
    document.getElementById('codBArticulo').value = articulos[pos_art].codBarras_art;
    document.getElementById('nomArticulo').value = articulos[pos_art].nombre_art;
    document.getElementById('descArticulo').value = articulos[pos_art].descripcion_art;
    document.getElementById('precArticulo').value = articulos[pos_art].precio_art;
    document.getElementById('stockArticulo').value = articulos[pos_art].stock_art;
    document.getElementById('fechaCArticulo').textContent = formatearFechaDT(articulos[pos_art].fecha_creacion_art);
    document.getElementById('fechaMArticulo').textContent = formatearFechaDT(articulos[pos_art].fecha_modificacion_art);
    document.getElementById('usuarioCArticulo').textContent = articulos[pos_art].usuarioCreacion;
    document.getElementById('usuarioMArticulo').textContent = articulos[pos_art].usuarioModificacion;
    
    var provArt = document.getElementById('provArticulo');
    for(var i = 0; i < provArt.options.length; i++){
        if(provArt.options[i].value === articulos[pos_art].proveedor_art){
            provArt.options[i].selected = true;
            break;
        }
    }

    var tipoIva = document.getElementById('tipoIva');
    for(var i = 0; i < tipoIva.options.length; i++){
        if(tipoIva.options[i].value === articulos[pos_art].iva_art){
            tipoIva.options[i].selected = true;
            break;
        }
    }

    var estArt = document.getElementById('estadoArticulo');
    for(var i = 0; i < estArt.options.length; i++){
        if(estArt.options[i].value === articulos[pos_art].estado_art){
            estArt.options[i].selected = true;
            break;
        }
    }

    var catArt = document.getElementById('catArticulo');
    for(var i = 0; i < catArt.options.length; i++){
        if(catArt.options[i].value === articulos[pos_art].categoria_art){
            catArt.options[i].selected = true;
            break;
        }
    }
}
...
function guardarArticulo(event){ 
    event.preventDefault();
    let codigoArticulo = parseInt(document.getElementById('codArticulo').value);
    let modificacion = buscarPosArticulo(codigoArticulo);
    
    if(modificacion !== -1){   //Si encuentra que ese codigo de articulo ya existe estinces se guarda como modifacado
        articulos[modificacion].codBarras_art = document.getElementById('codBArticulo').value;
        articulos[modificacion].nombre_art = document.getElementById('nomArticulo').value;
        articulos[modificacion].descripcion_art = document.getElementById('descArticulo').value;
        articulos[modificacion].categoria_art = document.getElementById('catArticulo').value;
        articulos[modificacion].proveedor_art = document.getElementById('provArticulo').value;
        articulos[modificacion].precio_art = parseInt(document.getElementById('precArticulo').value);
        articulos[modificacion].iva_art = document.getElementById('tipoIva').value;
        articulos[modificacion].stock_art = parseInt(document.getElementById('stockArticulo').value);
        articulos[modificacion].estado_art = document.getElementById('estadoArticulo').value;
        articulos[modificacion].fecha_modificacion_art = fechaHoraActualReturn();
        articulos[modificacion].usuarioModificacion = usuLogeado.usuario;
    ...
    }else{  //Se guarda como nuevo articulo
        let nuevoArticulo = {
            codigo_art: parseInt(document.getElementById('codArticulo').value),
            codBarras_art: document.getElementById('codBArticulo').value,
            nombre_art: document.getElementById('nomArticulo').value,
            descripcion_art: descripcion_art = document.getElementById('descArticulo').value, 
            categoria_art: document.getElementById('catArticulo').value, 
            proveedor_art: document.getElementById('provArticulo').value, 
            precio_art: parseInt(document.getElementById('precArticulo').value), 
            iva_art: document.getElementById('tipoIva').value, 
            stock_art: parseInt(document.getElementById('stockArticulo').value), 
            estado_art: document.getElementById('estadoArticulo').value, 
            fecha_creacion_art: fechaHoraActualReturn(), 
            fecha_modificacion_art: '',
...
async function eliminarFilaArticulos(idFila){
    var fila = document.getElementById('fila_' + idFila);
    var codArticulo = document.getElementById('codigo_' + idFila).textContent;
    var posEliminado = buscarPosArticulo(codArticulo);
    if(!vericarUsoArtVenta(codArticulo)){
        Swal.fire({
            ...
            if (result.isConfirmed) {        
                articulos.splice(posEliminado, 1);  //elimina el articulo en esa posicion
                localStorage.setItem('lSArticulos', JSON.stringify(articulos));
                // Eliminar la fila
                fila.remove();
                Swal.fire({
        ...
        Swal.fire({
            ...
            text: 'Este articulo esta en uso',
            icon: 'error',
            timer: 2000
        })
    }
...
function codArticuloCarga(){
    document.getElementById('labelAgregarModificar').textContent = "Cargar Nuevo Articulo";
    document.getElementById('formArticulos').reset();
    document.getElementById('codArticulo').value = codigoMayor() + 1;
    llenarSelectProveedores();
    llenarSelectCategorias();
}
...
function codigoMayor(){
    ...
    for(let x = 0; x < articulos.length; x++){
        if(x == 0){
            may = articulos[x].codigo_art;
        }else{
            if(articulos[x].codigo_art > may){
                may = articulos[x].codigo_art;
            }
        }
    }
...
function llenarSelectProveedores() {
    var select = document.getElementById("provArticulo");

    if(select.options.length < 1){
        proveedores.forEach(function(proveedores){
            var option = document.createElement("option");
            option.text = proveedores.nomProveedor;
            option.value = proveedores.nomProveedor;
...
function llenarSelectCategorias(){
    var select = document.getElementById("catArticulo");
    if(select.options.length < 1){
...
// Verifica si el articulo esta en uso en alguna venta
function vericarUsoArtVenta(cod){
    let uso = false;
    ventas.forEach((ventas) =>{
        ventas.codArt.forEach((codArt) => {
            if(codArt == cod){
                uso = true;
            }
        })
