 

// Seccion de recuperacion de datos de la base de datos 
let proveedores = JSON.parse(localStorage.getItem('lSProveedores'));
let articulos = JSON.parse(localStorage.getItem('lSArticulos'));
let compras = JSON.parse(localStorage.getItem('lSCompras'));
let usuLogeado = JSON.parse(sessionStorage.getItem('usuarioLogeado'));
let usuarios = JSON.parse(localStorage.getItem('lSUsuarios'));
let articulosPreCompra = JSON.parse(localStorage.getItem('lSCompra'));


// Variables bandera y otros
let tablaProveedoresIniciada = false;
let tablaArticulosIniciada = false;
let tablaCargaIniciada = false;
let artC = [];
let compraGuardada = false;
let tablaInformeComprasIniciada = false;
let infoCompras = [];

document.addEventListener('keydown', function(event) {
    // Verificar si la tecla F1 está presionada
    if (event.key === 'F1') {
        // Prevenir el comportamiento predeterminado (normalmente abre la ayuda del navegador)
        event.preventDefault();
        if (document.activeElement === document.getElementById('codProveedor')) {
            tablaBucarProveedoresCargada();
            $('#modalBuscarProveedores').modal('show');
        }else{
            if (document.activeElement === document.getElementById('codigoArticulo')) {
                $('#modalBuscarArticulos').modal('show');
            }
        }
    }
});


window.addEventListener("load", async () => {
    $('#modalIniciarSesion').modal('show');

    if (!sessionStorage.getItem('usuarioLogeado')) {
        Swal.fire({
            position: "center",
            icon: "info",
            title: "Inicio de sesión requerido",
            showConfirmButton: false,
            timer: 1500
        }).then(() => {
            window.location.href = 'index.html';
        });

    } else {
        await tablaArtCargada();
        await tablaBucarProveedoresCargada();
        await tablaBucarArticulosCargada();
        await tablaInfoComprasCargada();
        cargarDatosUsuario();
        nuevo();
        $('#modalIniciarSesion').modal('hide');
    }
});


function cargarDatosUsuario(){
    document.getElementById('usuarioLogeadoNavbar').textContent = usuLogeado.usuario;
    document.getElementById('nombreUsuario').textContent = usuLogeado.usuario;
    document.getElementById('nombreApellidoPerfil').textContent = usuLogeado.nombre + ' ' + usuLogeado.apellido;
    document.getElementById('correoPerfil').textContent = usuLogeado.correo;
    document.getElementById('telefonoPerfil').textContent = usuLogeado.telefono;
};
function editarUsuario(){
    document.getElementById('nombreEditUsuario').value = usuLogeado.nombre;
    document.getElementById('apellidoEditUsuario').value = usuLogeado.apellido;
    document.getElementById('correoEditUsuario').value = usuLogeado.correo;
    document.getElementById('telefonoEditUsuario').value = usuLogeado.telefono;
    document.getElementById('usuarioEditUsuario').value = usuLogeado.usuario;
    $('#modalEditarUsuario').modal('show');
}
const guardarEditUsuario = async(event) => {
    const forms = document.querySelectorAll('.needs-validation')
    Array.from(forms).forEach(form => {
        form.addEventListener('submit', event => {
        if (!form.checkValidity()) {
            event.preventDefault()
            event.stopPropagation()
        }

        form.classList.add('was-validated')
        }, false)
    })
    event.preventDefault();
    if(document.getElementById('formEditarUsuario').checkValidity()){
        let pos = buscarPosUsuario(usuLogeado.codigo);
        let contraseñaAnterior = document.getElementById('contAnterior').value;
        let nuevaC = document.getElementById('nuevaCont').value;
        let repetNuevaC = document.getElementById('repetirNuevaCont').value;
        if(contraseñaAnterior !== ''){
            if(contraseñaAnterior === usuLogeado.contraseña){
                if(nuevaC !== '' && nuevaC === repetNuevaC){
                    usuarios[pos].nombre = document.getElementById('nombreEditUsuario').value;
                    usuarios[pos].apellido = document.getElementById('apellidoEditUsuario').value;
                    usuarios[pos].correo = document.getElementById('correoEditUsuario').value;
                    usuarios[pos].usuario = document.getElementById('usuarioEditUsuario').value;
                    usuarios[pos].contraseña = nuevaC;
                    usuarios[pos].telefono = document.getElementById('telefonoEditUsuario').value;
                    Swal.fire({
                        icon: 'success',
                        title: 'Exito',
                        text: 'Usuario Guardado',
                        timer: 1500
                    })
    
                    usuLogeado = usuarios[pos];
                    sessionStorage.setItem('usuarioLogeado', JSON.stringify(usuLogeado));
                    localStorage.setItem('lSUsuarios', JSON.stringify(usuarios));
                    $('#modalEditarUsuario').modal('hide');
                }else{
                    Swal.fire({
                        title: 'Las contraseñas no coinciden',
                        icon: 'warning',
                        timer: 1500
                    })
                }
            }else{
                Swal.fire({
                    title: 'Contraseña Incorrecta',
                    text: 'Ingrese su contraseña anterior',
                    icon: 'error',
                    showConfirmButton: true,
                    timer: 2000
                })
            }
        }else{        
            usuarios[pos].nombre = document.getElementById('nombreEditUsuario').value;
            usuarios[pos].apellido = document.getElementById('apellidoEditUsuario').value;
            usuarios[pos].correo = document.getElementById('correoEditUsuario').value;
            usuarios[pos].usuario = document.getElementById('usuarioEditUsuario').value;
            usuarios[pos].telefono = document.getElementById('telefonoEditUsuario').value;
            console.log(usuarios);
    
            usuLogeado = usuarios[pos];
            sessionStorage.setItem('usuarioLogeado', JSON.stringify(usuLogeado));
    
            localStorage.setItem('lSUsuarios', JSON.stringify(usuarios));
            await Swal.fire({
                title: 'Usuario Guardado',
                icon: 'success',
                timer: 1500
            })
            $('#modalEditarUsuario').modal('hide');
        }
        cargarDatosUsuario();
    }

}
function buscarPosUsuario(cod){
    pos = -1;
    usuarios.forEach((usuarios, index) => {
        if(usuarios.codigo == cod){
            pos = index;
        }
    })
    return pos;
}

function nuevo(){
    desabilitarCamposCompras(false);
    document.getElementById('formProveedor').reset();
    document.getElementById('formCargarArticulo').reset();
    artC = [];
    
    tablaArtCargada();
    document.getElementById('codCompra').value = codigoMayor() + 1;  
    
    document.getElementById('usuarioCreacionCompra').textContent = '';
    document.getElementById('fechaCreacionCompra').textContent = '';
    document.getElementById('btnAnular').hidden = true;
    
    compraGuardada = false;
    document.getElementById('fechaComprobante').focus();
}

function codigoMayor(){
    let may = 0;
    for(let x = 0; x < compras.length; x++){
        if(x == 0){
            may = compras[x].codCompra;
        }else{
            if(compras[x].codCompra > may){
                may = compras[x].codCompra;
            }
        }
    }
    return may;
};


const opcionesDataTableProveedores = {
    pageLength: 5,
    destroy: true,
    dom: 'p',
    buttons: [
        {
            extend: 'excelHtml5',
            text: '<i class="bi bi-file-earmark-excel-fill"></i>',
            titleAttr: 'Exportar a Excel',
            className: 'btn btn-success'
        },
        {
            extend: 'pdfHtml5',
            text: '<i class="bi bi-file-earmark-pdf-fill"></i>',
            titleAttr: 'Exportar a PDF',
            title: 'Reporte de Compras',
            className: 'btn btn-danger',
            messageTop: '',
            messageBottom: '',
            customize: function (doc) {
                console.log(doc);
                // Agregar logo en la cabecera
                doc.content.splice(0, 0, {
                    margin: [0, 0, 0, 12],
                    alignment: 'center',
                    image: 'data:image/jpeg;base64,',  // Reemplaza con tu imagen en base64
                });

                doc.footer = function(currentPage, pageCount) {
                    return {
                        text: currentPage.toString() + ' de ' + pageCount,
                        alignment: 'center',
                        style: 'footerStyle'
                    };
                };
                doc.styles.footerStyle = {
                    fontSize: 10,
                    margin: [0, 0, 0, 10]
                };
            }
        },
        {
            extend: 'print',
            text: '<i class="bi bi-printer-fill"></i>',
            titleAttr: 'Imprimir',
            className: 'btn btn-info'
        },
    ],
    language:{
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};

const tablaBucarProveedoresCargada = async() => {
        // Datatable Buscar Cliente
    if(tablaProveedoresIniciada){
        dataTableProveedores.destroy();
    }
    await mostrarDatosProveedores();
    dataTableProveedores = $("#tablaBuscarProveedores").DataTable(opcionesDataTableProveedores);
    //vincula el input buscar cliente a la funcion de busqueda de datatables
    $('#buscarProveedores').on('keyup', function() {
        dataTableProveedores.search(this.value).draw();
    });
    tablaProveedoresIniciada = true;
}

const mostrarDatosProveedores = async =>{
    try{        
        let contensBody = ``;
        proveedores.forEach((proveedores, index) => {
            let id = index + 1;
            contensBody+= `
            <tr data-bs-dismiss="modal" onclick="document.getElementById('codProveedor').value = ${proveedores.codProveedor}, buscarProveedor()" id="fila_` + id + `">
                <td hidden>${index + 1}</td>
                <td id="codigoCliente_` + id + `">${proveedores.codProveedor}</td>
                <td>${proveedores.nomProveedor}</td>
                <td>${proveedores.rucProveedor}</td>
                <td>${proveedores.dirProveedor}</td>
                <td>${proveedores.telProveedor}</td>
                <td>${proveedores.correoProveedor}</td>
            </tr>`;
            tablaBuscarProveedoresBody.innerHTML = contensBody; 
        });
    }catch(ex){
        alert(ex);
        console.log('No se pudo cargar');
    }
};

function buscarProveedor(){
    let codProveedor = document.getElementById('codProveedor').value;
    if(codProveedor !== ''){
        posProveedor = buscarPosProveedor(codProveedor);

        if(posProveedor !== -1){
            document.getElementById('rucProveedor').value = proveedores[posProveedor].rucProveedor;
            document.getElementById('nomRazonProveedor').value = proveedores[posProveedor].nomProveedor;
        }else{
            document.getElementById('rucProveedor').value = '';
            document.getElementById('nomRazonProveedor').value = '';
        }
    }else{
        document.getElementById('rucProveedor').value = '';
        document.getElementById('nomRazonProveedor').value = '';
    }
};

function buscarPosProveedor(cod){
    let pos = -1;
    for(let x = 0; x < proveedores.length; x++){
        if(cod == proveedores[x].codProveedor){
            pos = x;
            break;
        }
    }
    return pos;
};




// Buscar Articulo
const opcionesDataTableArticulos = {
    pageLength: 5,
    destroy: true,
    dom: 'p',
    language:{
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};

const tablaBucarArticulosCargada = async() => {
    if(tablaArticulosIniciada){
        dataTableProveedores.destroy();
    }
    await mostrarDatosArticulos();
    dataTableArticulos = $("#tablaBuscarArticulos").DataTable(opcionesDataTableArticulos);
    //vincula el input buscar articulo a la funcion de busqueda de datatables
    $('#buscarArticulos').on('keyup', function() {
        dataTableArticulos.search(this.value).draw();
    });
    tablaArticulosIniciada = true;
}

const mostrarDatosArticulos = async =>{
    try{        
        let contensBody = ``;
        articulos.forEach((articulos, index) => {
            let id = index + 1;
            contensBody+= `
            <tr data-bs-dismiss="modal" onclick="articuloSeleccionado(${articulos.codigo_art}), buscarArticulo()">
                <td hidden>${index + 1}</td>
                <td>${articulos.codigo_art}</td>
                <td>${articulos.nombre_art}</td>
                <td>${articulos.descripcion_art}</td>
                <td>${articulos.categoria_art}</td>
                <td>${articulos.proveedor_art}</td>
                <td>${articulos.precio_art}</td>
            </tr>`;
            tablaBuscarArticulosBody.innerHTML = contensBody; 
        });
    }catch(ex){
        alert(ex);
        console.log('No se pudo cargar');
    }
};

function articuloSeleccionado(cod){
    document.getElementById('codigoArticulo').value = cod;
    document.getElementById('cantidadArticulo').focus();
}

function buscarArticulo(){
    let codArticulo = document.getElementById('codigoArticulo').value;
    if(codArticulo !== ''){
        posArticulo = buscarPosArticulo(codArticulo);

        if(posArticulo !== -1){
            document.getElementById('nombreArticulo').value = articulos[posArticulo].nombre_art;
        }else{
            document.getElementById('nombreArticulo').value = '';
        }
    }else{
        document.getElementById('nombreArticulo').value = '';
    }
};

function buscarPosArticulo(cod){
    let pos = -1;
    for(let x = 0; x < articulos.length; x++){
        if(cod == articulos[x].codigo_art){
            pos = x;
            break;
        }
    }
    return pos;
};

function cargarArticulo(event){
    event.preventDefault();
    let codigo = document.getElementById('codigoArticulo').value;
    let cantidad = document.getElementById('cantidadArticulo').value;
    let precio = document.getElementById('precioUnitario').value;
    
    
    if(codigo !== '' && parseInt(codigo) !== 0){
        let posArt = buscarPosArticulo(codigo);
        if(articulos[posArt].estado_art == 'Activo'){
            let nuevaCarga = {
                codArtCarga: parseInt(codigo),
                nomArtCarga: articulos[posArt].nombre_art,
                cantArtCarga: parseInt(cantidad),
                precArtCarga: precio,
                ivaArtCarga: articulos[posArt].iva_art
            }   
            artC.push(nuevaCarga);
            console.log(artC)
            tablaArtCargada();

            document.getElementById('formCargarArticulo').reset();        
            document.getElementById('codigoArticulo').focus();
        }else{
            Swal.fire({
                position: "center",
                icon: "info",
                title: "Articulo Inactivo",
                showConfirmButton: true,
                timer: 2000
            }).then((result) =>{
                document.getElementById('formCargarArticulo').reset();
                document.getElementById('codigoArticulo').focus();
            })
        }
            
    }
};

function calcularTotal(){
    let cantArt = parseInt(document.getElementById('cantidadArticulo').value);
    let precioArt = parseInt(document.getElementById('precioUnitario').value);
    if(cantArt !== '' && precioArt !== ''){
        let total = cantArt * precioArt;
        document.getElementById('subTotal').value = total.toLocaleString();
    }
}
async function eliminarFilaCarga(pos){
    if(compraGuardada == false){
        artC.splice(pos, 1);
        tablaArtCargada();
        console.log(artC);
    }
    
}


// Mostrar articulos cargados
const tablaArtCargada = async()=>{
    if(tablaCargaIniciada){
        dataTableCarga.destroy();
    }
    await mostrarDatosCarga();
    dataTableCarga = $("#tablaCargaArticulos").DataTable(opcionesDataTableCarga);
    tablaCargaIniciada = true;
};

const opcionesDataTableCarga = {
    destroy: true,  
    dom: '', 
    language:{
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};

const mostrarDatosCarga = async () => {
    try {        
        let contensBody = ``;
        totalExenta = 0, total5 = 0, total10 = 0;
        totalVenta = 0;
        
        if(compraGuardada == true){
            artC.forEach((artC, index) => {
                let id = index + 1;
                let iva10 = '', iva5 = '', exenta = '';
                parseInt(total5, total10, totalExenta);
                if(artC.ivaArtCarga == '10%'){
                    iva10 = artC.precArtCarga * artC.cantArtCarga;
                    total10 = parseInt(total10) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                }else{
                    if(artC.ivaArtCarga == '5%'){
                        iva5 = artC.precArtCarga * artC.cantArtCarga;
                        total5 = parseInt(total5) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                    }else{
                        exenta = artC.precArtCarga * artC.cantArtCarga;
                        totalExenta = parseInt(totalExenta) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                    }
                }
                totalVenta = totalVenta + (artC.precArtCarga * artC.cantArtCarga);
    
                contensBody += `
                <tr id="filaCarga_${id}">
                    <td class="border" id="pos_${id}">${id}</td>
                    <td class="border" id="nomArtCarga_${id}">${artC.nomArtCarga}</td>
                    <td class="text-end border" id="cantArtCarga_${id}">${artC.cantArtCarga}</td>
                    <td class="text-end border" id="preArtCarga_${id}">${artC.precArtCarga.toLocaleString()}</td>
                    <td class="text-end border" id="exentaArtCarga_${id}">${exenta.toLocaleString()}</td>
                    <td class="text-end border" id="iva5ArtCarga_${id}">${iva5.toLocaleString()}</td>
                    <td class="text-end border" id="iva10ArtCarga_${id}">${iva10.toLocaleString()}</td>
                    <td class="centered"></td>
                </tr>`;
            });
        }else{
            artC.forEach((artC, index) => {
                let id = index + 1;
                let iva10 = '', iva5 = '', exenta = '';
                if(artC.ivaArtCarga == '10%'){
                    iva10 = artC.precArtCarga * artC.cantArtCarga;
                    total10 = parseInt(total10) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                }else{
                    if(artC.ivaArtCarga == '5%'){
                        iva5 = artC.precArtCarga * artC.cantArtCarga;
                        total5 = parseInt(total5) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                    }else{
                        exenta = artC.precArtCarga * artC.cantArtCarga;
                        totalExenta = parseInt(totalExenta) + (parseInt(artC.precArtCarga) * artC.cantArtCarga);
                    }
                }
                totalVenta = totalVenta + (artC.precArtCarga * artC.cantArtCarga);
                
                contensBody += `
                <tr id="filaCarga_${id}">
                    <td class="border" id="pos_${id}">${id}</td>
                    <td class="border" id="nomArtCarga_${id}">${artC.nomArtCarga}</td>
                    <td class="text-end border" id="cantArtCarga_${id}">${artC.cantArtCarga}</td>
                    <td class="text-end border" id="preArtCarga_${id}">${artC.precArtCarga.toLocaleString()}</td>
                    <td class="text-end border" id="exentaArtCarga_${id}">${exenta.toLocaleString()}</td>
                    <td class="text-end border" id="iva5ArtCarga_${id}">${iva5.toLocaleString()}</td>
                    <td class="text-end border" id="iva10ArtCarga_${id}">${iva10.toLocaleString()}</td>
                    <td class="centered">
                        <button onclick="eliminarFilaCarga(${index});" class="btn btn-danger bi bi-trash3"></button>
                    </td>
                </tr>`;
            });
        }
        totalVentaCarga = totalVenta;

        tablaCargaBody.innerHTML = contensBody;
        
        let contentFoot = `
            <tr>
                <td class="text-start border" colspan="4">Sub Total</td>
                <td class="text-end border">${totalExenta.toLocaleString()}</td>
                <td class="text-end border">${total5.toLocaleString()}</td>
                <td class="text-end border">${total10.toLocaleString()}</td>
                <td class="centered border"></td>
            </tr>
        `;
        tablaCargaFoot.innerHTML = contentFoot;

        let contentFoot2 = `
            <tr>
                <td class="text-start border border-2 border-black fs-5" colspan="5">Total a pagar</td>
                <td class="text-end border border-2 border-black fs-5" colspan="3" id="subTotalCargado">${totalVenta.toLocaleString()}</td>
            </tr>
        `;
        tablaCargaFoot.insertAdjacentHTML('beforeend', contentFoot2);
        let contentFoot3 = `
            <tr>
                <td class="text-start border border-black" colspan="2">Liquidacion de IVA</td>
                <td class="text-end border border-black" colspan="2">${calcularIva5(total5).toLocaleString()}</td>
                <td class="text-end border border-black" colspan="2">${calcularIva10(total10).toLocaleString()}</td>
                <td class="text-end border border-black" colspan="2">${(calcularIva10(total10) + calcularIva5(total5)).toLocaleString()}</td>
            </tr>
        `;

        tablaCargaFoot.insertAdjacentHTML('beforeend', contentFoot3);
    } catch (ex) {
        console.log('No se pudo cargar los articulos', ex);
    }
};

function calcularIva5(valor){
    let iva = Math.ceil(valor / 21);
    return iva;
}
function calcularIva10(valor){
    let iva = Math.ceil(valor / 11);
    return iva;
}






// Seccion de finalizar compra
function finalizarCompra(event){
    event.preventDefault();
    if(compraGuardada == false){
        if(artC.length >= 1){
            let codArticulo = [];
            let preUnit = [];
            let cantArticulo = [];
            let ivaArt = [];

            artC.forEach((artC, index) =>{
                let posArt = buscarPosArticulo(artC.codArtCarga);
                articulos[posArt].stock_art += artC.cantArtCarga;
                codArticulo[index] = artC.codArtCarga;
                preUnit[index] = parseInt(artC.precArtCarga);
                cantArticulo[index] = artC.cantArtCarga;
                ivaArt[index] = artC.ivaArtCarga;
            });
            
            let nuevaCompra = {
                codCompra: parseInt(document.getElementById('codCompra').value),
                codProveedor:parseInt(document.getElementById('codProveedor').value), 
                codArtCompra: codArticulo,
                precioUnitCompra: preUnit,
                cantArtCompra: cantArticulo,
                ivaArtCompra: ivaArt,
                numTimbradoCompra: document.getElementById('numTimbrado').value,
                numFacturaCompra: document.getElementById('numFactura').value,
                fechaComprobante: document.getElementById('fechaComprobante').value,
                nombreRazonSocialCompra: document.getElementById('nomRazonProveedor').value,
                rucCompra: document.getElementById('rucProveedor').value,
                condicionCompra: document.getElementById('condiCompra').value,
                estadoCompra: true,
                fechaHoraCreacionCompra: fechaHoraActualReturn(),
                usuarioCreacionCompra: usuLogeado.usuario
            }

            compras.push(nuevaCompra);    // Actualiza compra local
            localStorage.setItem('lSCompras', JSON.stringify(compras));       // Actualiza ventas de localstorage
            localStorage.setItem('lSArticulos', JSON.stringify(articulos));       // Actualiza los articulos del localstorage
            compraGuardada = true;
            if(usuLogeado.rol == 'Admin'){
                document.getElementById('btnAnular').hidden = false;
            }
            Swal.fire({
                position: "center",
                icon: "success",
                title: "Exito",
                text: "Compra Guardada",
                showConfirmButton: true,
                timer: 2000
            })
        }else{   
            document.getElementById('formCargarArticulo').reportValidity();
        }
    }
};
function fechaHoraActualReturn() {
    var fechaActual = new Date();

    // Obtener la fecha en formato YYYY-MM-DD
    var year = fechaActual.getFullYear();
    var month = (fechaActual.getMonth() + 1).toString().padStart(2, '0');
    var day = fechaActual.getDate().toString().padStart(2, '0');

    // Obtener la hora en formato HH:MM
    var hours = fechaActual.getHours().toString().padStart(2, '0');
    var minutes = fechaActual.getMinutes().toString().padStart(2, '0');

    // Formatear la fecha y hora como YYYY-MM-DDTHH:MM
    var fechaHora = `${year}-${month}-${day}T${hours}:${minutes}`;

    // Establecer el valor del input
    return fechaHora;
};
async function cerrarSesion(){
    Swal.fire({
        title: "Cerrar Sesion?",
        text: "",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#d33",
        cancelButtonColor: "#3085d6",
        confirmButtonText: "Si",
        cancelButtonText: "No"
    }).then((result) => {
        if (result.isConfirmed) {
            window.location.href = 'index.html';
            sessionStorage.clear();
        }
    });
}
function volver(){  
    let user = JSON.parse(sessionStorage.getItem('usuarioLogeado'));
    switch (user.rol) {
        case 'Admin':
            window.location.href = 'menuAdmin.html';
            break;
        case 'Cajero':
                window.location.href = 'menuCajero.html';
            break;
        case 'Stock':
                window.location.href = 'menuStock.html';
            break;
        default:
            // Código para el caso por defecto
    }
};
function anularCompra(){
    Swal.fire({
        title: "Eliminar Compra?",
        text: "No se puede revertir!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: "Si",
        cancelButtonText: "No"
    }).then((result) => {
        if (result.isConfirmed) {
            console.log('anular compra');
            let codCompra = document.getElementById('codCompra').value;
            let posCompra = buscarPosCompras(codCompra);

            // Tiene que volver a restar del stock los articulos de la compra anulada
            for(let x = 0; x < compras[posCompra].codArtCompra.length; x++){
                let pos = buscarPosArticulo(compras[posCompra].codArtCompra[x]);
                articulos[pos].stock_art -= compras[posCompra].cantArtCompra[x];
            }
            compras.splice(posCompra, 1);

            localStorage.setItem('lSCompras', JSON.stringify(compras));
            localStorage.setItem('lSArticulos', JSON.stringify(articulos));

            Swal.fire({
                title: "Compra eliminada Eliminada!",
                icon: "success",
                timer: 1500
            });
            nuevo();
        }
    })
};
function buscarPosCompras(cod){
    let pos = -1;
    for(let x=0; x < compras.length; x++){
        if(cod == compras[x].codCompra){
            pos = x;
            break;
        }
    }
    return pos;
}
function compraAnterior(num){
    try{
        let codCompra = document.getElementById('codCompra').value;
        if(codCompra == 1 || codCompra - num == 0){
        }else{
            let posCompra = buscarPosCompras(codCompra - num);
            if(compras[posCompra].estadoCompra == true){
                mostrarCompra(posCompra);
            }else{
                compraAnterior(num + 1);
            }
        } 
    }catch(error){
        console.log(error);
    }
}
function compraPosterior(num){
    try{        
        let codCompra = parseInt(document.getElementById('codCompra').value) + num;
        let posCompra = buscarPosCompras(codCompra);
        if(posCompra == -1 || posCompra >= compras.length){
        }else{
            if(compras[posCompra].estadoCompra == true){
                mostrarCompra(posCompra);
            }else{
                compraPosterior(num + 1);
            }
        }
    }catch(error){
        console.log(error);
    }
}

function mostrarCompra(pos){
    compraGuardada = true;
    desabilitarCamposCompras(true);
    document.getElementById('codCompra').value = compras[pos].codCompra;
    document.getElementById('fechaComprobante').value = compras[pos].fechaComprobante;

    document.getElementById('codProveedor').value = compras[pos].codProveedor;
    document.getElementById('rucProveedor').value = compras[pos].rucCompra;
    document.getElementById('nomRazonProveedor').value = compras[pos].nombreRazonSocialCompra;

    let condicion = document.getElementById('condiCompra');  
    for(let x = 0; x < condicion.options.length; x++){
        if(condicion.options[x].value === compras[pos].condicionCompra){
            condicion.options[x].selected = true;
            break;
        }
    }
    document.getElementById('numTimbrado').value = compras[pos].numTimbradoCompra;
    document.getElementById('numFactura').value = compras[pos].numFacturaCompra;
    document.getElementById('usuarioCreacionCompra').textContent = compras[pos].usuarioCreacionCompra;
    document.getElementById('fechaCreacionCompra').textContent = formatearFechaDT(compras[pos].fechaHoraCreacionCompra);
    
    artC = [];
    for(let x = 0; x < compras[pos].codArtCompra.length; x++){
        let posArt = buscarPosArticulo(compras[pos].codArtCompra[x]);
        let registroCarga = {
            codArtCarga: compras[pos].codArtCompra[x],
            nomArtCarga: articulos[posArt].nombre_art,
            cantArtCarga: compras[pos].cantArtCompra[x],
            precArtCarga: compras[pos].precioUnitCompra[x],
            ivaArtCarga: compras[pos].ivaArtCompra[x]
        }
        artC.push(registroCarga);
    }
    if(usuLogeado.rol == 'Admin'){
        document.getElementById('btnAnular').hidden = false;
    }
    tablaArtCargada();    
}

function desabilitarCamposCompras(valor){
    document.getElementById('fechaComprobante').disabled = valor;
    document.getElementById('codProveedor').disabled = valor;
    document.getElementById('condiCompra').disabled = valor;
    document.getElementById('numTimbrado').disabled = valor;
    document.getElementById('numFactura').disabled = valor;
    document.getElementById('formCargarArticulo').disabled = valor;
    document.getElementById('btnGuardar').disabled = valor;
}
function formatearFecha(date) {
    let fecha = new Date(date);

    let dia = ('0' + fecha.getDate()).slice(-2); // Añade un cero inicial si es necesario
    let mes = ('0' + (fecha.getMonth() + 1)).slice(-2); // Los meses comienzan desde 0, por lo que se añade 1
    let año = fecha.getFullYear();

    return `${dia}/${mes}/${año}`;
}

function formatearFechaDT(datetimeLocal) {
    let fechaHora = new Date(datetimeLocal);

    let dia = ('0' + fechaHora.getDate()).slice(-2); // Añade un cero inicial si es necesario
    let mes = ('0' + (fechaHora.getMonth() + 1)).slice(-2); // Los meses comienzan desde 0, por lo que se añade 1
    let año = fechaHora.getFullYear();
    let horas = ('0' + fechaHora.getHours()).slice(-2);
    let minutos = ('0' + fechaHora.getMinutes()).slice(-2);

    return `${dia}/${mes}/${año} ${horas}:${minutos}`;
}




const tablaInfoComprasCargada = async() =>{
    if(tablaInformeComprasIniciada){
        dataTableInfoCompras.destroy();
    }
    await mostrarDatosInfoCompras();
    dataTableInfoCompras = $("#tablaInformeCompras").DataTable(opcionesDataTableInfoCompras);
     // Detectar cuando se hace clic en el botón de imprimir
    dataTableInfoCompras.buttons().container().on('click', '.buttons-print', function() {
        funcionesPrint();
    });
    tablaInformeComprasIniciada = true;
};

const opcionesDataTableInfoCompras = {
    destroy: true,  
    lengthMenu: [10, 20, 50, 100],
    dom: 'Bfrltip', 
    buttons: [
        {
            extend: 'excelHtml5',
            text: '<i class="bi bi-file-earmark-excel-fill"></i>',
            titleAttr: 'Exportar a Excel',
            className: 'btn btn-success'
        },
        {
            extend: 'pdfHtml5',
            text: '<i class="bi bi-file-earmark-pdf-fill"></i>',
            titleAttr: 'Exportar a PDF',
            title: 'Reporte de Compras',
            className: 'btn btn-danger',
            messageTop: '',
            messageBottom: '',
            customize: function (doc) {
                doc.footer = function(currentPage, pageCount) {
                    return {
                        text: currentPage.toString() + ' de ' + pageCount,
                        alignment: 'center',
                        style: 'footerStyle'
                    };
                };
                doc.styles.footerStyle = {
                    fontSize: 10,
                    margin: [0, 0, 0, 10]
                };
                // Agregar logo en la cabecera
                doc.content.splice(0, 0, {
                    margin: [0, 0, 0, 12],
                    alignment: 'center',
                    image: 'data:image/png'
                });
            }
        },
        {
            extend: 'print',
            text: '<i class="bi bi-printer-fill"></i>',
            titleAttr: 'Imprimir',
            className: 'btn btn-info'
        }
    ],
    language: {
        decimal: "",
        emptyTable: "No hay registros",
        info: "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
        infoEmpty: "Mostrando 0 to 0 of 0 Entradas",
        infoFiltered: "(Filtrado de _MAX_ total entradas)",
        infoPostFix: "",
        thousands: ",",
        lengthMenu: "Mostrar _MENU_ Entradas",
        loadingRecords: "Cargando...",
        processing: "Procesando...",
        search: "Buscar:",
        zeroRecords: "Sin resultados encontrados",
        paginate: {
            first: "Primero",
            last: "Ultimo",
            next: "Siguiente",
            previous: "Anterior",
        },
    }
};


const mostrarDatosInfoCompras = async () => {
    try {        
        let contensBody = ``;
        compras.forEach((compras, index) => {
            let id = index + 1;

            
            let totalGeneral = 0;
            for(let x = 0; x < compras.codArtCompra.length; x++){
                let total = compras.cantArtCompra[x] * compras.precioUnitCompra[x];
                totalGeneral = totalGeneral + total;
            }
            
            contensBody += `
            <tr id="fila_${id}]">
                <td class="bg-primary-200 text-100 centered" >${id}</td>
                <td class="bg-primary-200 text-100 centered" id="codigoCompra_${id}">${compras.codCompra}</td>
                <td class="bg-primary-200 text-100 centered">${compras.nombreRazonSocialCompra}</td>
                <td class="bg-primary-200 text-100 centered">${formatearFecha(compras.fechaComprobante)}</td>
                <td class="bg-primary-200 text-100 centered">${compras.numFacturaCompra}</td>
                <td class="bg-primary-200 text-100 centered">${totalGeneral.toLocaleString()}</td>
                <td class="bg-primary-200 text-100 centered">${formatearFechaDT(compras.fechaHoraCreacionCompra)}</td>
                <td class="bg-primary-200 text-100 centered">${compras.usuarioCreacionCompra}</td>
                <td class="bg-primary-200 text-100 centered">
                    <button id="btnAccionInfoVenta" onclick="verCompra(codigoCompra_${id})" class="btn btn-success "><i class="bi bi-eye-fill"></i></button></td>
            </tr>`;
        });
        tablaInformeComprasBody.innerHTML =contensBody;
    } catch (ex) {
        alert(ex);
        console.log('No se pudo cargar la planilla');
    }
};

function verCompra(id){
    $('#offcanvasDarkNavbar').offcanvas('hide');    
    let codCompra = id.textContent;
    let posCompra = buscarPosCompras(codCompra);
    mostrarCompra(posCompra);
    $('#modalInformeCompras').modal('hide');    
}


