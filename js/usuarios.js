...
const opcionesDataTableUsuarios = {
    columnDefs:[{className:"centered border bg-primary-200 text-white", targets:[0,1,2,3,4,5]}],
...
const tablaUsuariosCargada = async()=>{
    if(tablaIniciada){
        dataTable.destroy();
    }
    await mostrarDatos();

    dataTable = $("#tablaUsuarios").DataTable(opcionesDataTableUsuarios);
...
const mostrarDatos = async =>{
    try{        
        ...
        usuarios.forEach((usuarios, index) => {
            ...
            contensBody+= `
            <tr id="fila_${id}">
                <td class="centered border bg-primary-200 text-white" id="codigo_` + id + `">${usuarios.codigo}</td>
                <td class="centered border bg-primary-200 text-white">${usuarios.nombre} ${usuarios.apellido}</td>
                <td class="centered border bg-primary-200 text-white">${usuarios.telefono}</td>
                <td class="centered border bg-primary-200 text-white">${usuarios.correo}</td>
                <td class="centered border bg-primary-200 text-white">${usuarios.usuario}</td>
                <td class="centered border bg-primary-200 text-white">${usuarios.rol}</td>
                <td class="centered border bg-primary-200 text-white">${usuarios.estado}</td>
                <td class="centered border bg-primary-200 text-white">${formatearFechaDT(usuarios.fechaCreacion)}</td>
                <td class="centered border bg-primary-200 text-white">${usuarios.usuarioCreacion}</td>
                <td class="centered border bg-primary-200">
                    <button onclick="editarFilaUsuario(${id});" data-bs-toggle="modal" data-bs-target="#modalUsuarios" class="btn btn-success bi bi-pencil-square"></button>
                    <button onclick="eliminarFilaUsuario(${id});" class="btn btn-danger bi bi-trash3"></button>
                </td>
            </tr>`;
            tablaUsuariosBody.innerHTML = contensBody; 
        });
    }catch(ex){
        alert(ex);
...
function editarFilaUsuario(idFila){
    document.getElementById('labelAgregarModificar').textContent = "MODIFICAR USUARIO";
    document.getElementById('btnRestablecer').hidden = false;
    var codUsuario = document.getElementById('codigo_' + idFila).textContent;
    var pos = buscarPosUsuario(codUsuario);
    document.getElementById('codUsuarioForm').value = usuarios[pos].codigo;
    document.getElementById('nombreUsuarioForm').value = usuarios[pos].nombre;
    document.getElementById('apellidoUsuarioForm').value = usuarios[pos].apellido;
    document.getElementById('telUsuarioForm').value = usuarios[pos].telefono;
    document.getElementById('correoUsuarioForm').value = usuarios[pos].correo;
    document.getElementById('nUsuarioForm').value = usuarios[pos].usuario;
    document.getElementById('fechaCUsuarioForm').textContent = formatearFechaDT(usuarios[pos].fechaCreacion);
    document.getElementById('fechaMUsuarioForm').textContent = formatearFechaDT(usuarios[pos].fechaModificacion);
    document.getElementById('usuarioCUsuarioForm').textContent = usuarios[pos].usuarioCreacion;
    document.getElementById('usuarioMUsuarioForm').textContent = usuarios[pos].usuarioModificacion;

    let selecRol = document.getElementById('rolUsuarioForm');
    for(let x = 0; x < selecRol.options.length; x++){
        if(selecRol.options[x].value === usuarios[pos].rol){
            selecRol.options[x].selected = true;
            break;
        }
    }
    let selecEstado = document.getElementById('estadoUsuarioForm');
    for(let x = 0; x < selecEstado.options.length; x++){
        if(selecEstado.options[x].value === usuarios[pos].estado){
            selecEstado.options[x].selected = true;
            break;
        }
    }
}
...
async function guardarUsuario(event){ 
    ...
    if(document.getElementById('formUsuarios').checkValidity()){
        let codiUsuario = parseInt(document.getElementById('codUsuarioForm').value);
        let modificacion = buscarPosUsuario(codiUsuario);
    
        if(modificacion !== -1){   //Si encuentra que ese codigo de la categoria ya existe estonces se guarda como modifacado
            if(document.getElementById('rolUsuarioForm').value !== 'Admin'){
                if(verificarUsuario() >= 1){
                    usuarios[modificacion].nombre = (document.getElementById('nombreUsuarioForm').value).toUpperCase();
                    usuarios[modificacion].apellido = (document.getElementById('apellidoUsuarioForm').value).toUpperCase();
                    usuarios[modificacion].usuario = (document.getElementById('nUsuarioForm').value).toLowerCase();
                    usuarios[modificacion].correo = (document.getElementById('correoUsuarioForm').value).toLowerCase();
                    usuarios[modificacion].telefono = document.getElementById('telUsuarioForm').value;
                    usuarios[modificacion].rol = document.getElementById('rolUsuarioForm').value;
                    usuarios[modificacion].estado = document.getElementById('estadoUsuarioForm').value;
                    usuarios[modificacion].fechaModificacion = fechaHoraActualReturn();
                    usuarios[modificacion].usuarioModificacion = usuLogeado.usuario;
                    ...
                    Swal.fire({
                        ...
                        text: 'Se requiere almenos un administrador',
                        icon: 'error',
                        timer: 2000
                    })
                } 
        ...
        }else{  //Se guarda como nuevo usuario
            let nuevoUsuario = {
                codigo: parseInt(document.getElementById('codUsuarioForm').value),
                nombre: (document.getElementById('nombreUsuarioForm').value).toUpperCase(),
                apellido: (document.getElementById('apellidoUsuarioForm').value).toUpperCase(),
                usuario: (document.getElementById('nUsuarioForm').value).toLowerCase(),
                correo: (document.getElementById('correoUsuarioForm').value).toLowerCase(),
                telefono: document.getElementById('telUsuarioForm').value,
                contraseña: '123',
                rol: document.getElementById('rolUsuarioForm').value,
                estado: document.getElementById('estadoUsuarioForm').value,
...
const restablecerContr = async() => {
    var codUsuario = document.getElementById('codUsuarioForm').value;
    var pos = buscarPosUsuario(codUsuario);
    Swal.fire({
        title: 'Restablecer Contraseña?',
        text: 'La contraseña por defecto es 123',
        icon: 'warning',
        showConfirmButton: true,
        showCancelButton: true,
        cancelButtonColor: '#d33',
        confirmButtonColor: "#3085d6",
        confirmButtonText: 'Si, Restablcer',
    }).then((result) => {
        if(result.isConfirmed){
            usuarios[pos].contraseña = '123';
            localStorage.setItem('lSUsuarios', JSON.stringify(usuarios));
            Swal.fire({
                title: 'Exito',
...
async function eliminarFilaUsuario(idFila){
    ...
    if(verificarUsuario() >= 1){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                    usuarios.splice(pos, 1);  //elimina al usuario en esa posicion
                    localStorage.setItem('lSUsuarios', JSON.stringify(usuarios));
        ...
        Swal.fire({
            ...
            text: "No puedes borrar al unico usuario administrador",
            icon: "error",
            showConfirmButton: true,
            timer: 2000
        })
...
function nuevoUsuario(){
    document.getElementById('btnRestablecer').hidden = true;
    document.getElementById('codUsuarioForm').value = codigoMayor() + 1;
    document.getElementById('labelAgregarModificar').textContent = 'AGREGAR USUARIO';
...
function codigoMayor(){
    ...
    for(let x = 0; x < usuarios.length; x++){
        if(x == 0){
            may = usuarios[x].codigo;
        }else{
            if(usuarios[x].codigo > may){
                may = usuarios[x].codigo;
            }
        }
    }
